const navItems = [
  ...
  { id: "builder", label: "Clip Builder", icon: "scissors", group: "core", count: "builder", tone: "info" },
...
const state = {
  ...
  studioSeek: null,
  studioBusy: false,
  builderTool: localStorage.getItem("builderTool") || "ai",
  builderInspectorTab: localStorage.getItem("builderInspectorTab") || "inspector",
  builderEdit: null,
  ...
  agentRunBusy: false,
  agent101Sessions: [],
  activeAgent101SessionId: localStorage.getItem("activeAgent101SessionId") || "",
  agent101Session: null,
  agent101Events: [],
  agent101Busy: false,
  agent101EventSource: null,
  agent101FileModal: null,
  capcutAgentSessions: [],
  capcutAgentBusy: false,
  capcutAgentEventSource: null,
  activeCapcutAgentSessionId: localStorage.getItem("activeCapcutAgentSessionId") || "",
  refreshBusy: false,
  agentChatOpen: false,
  ...
  scoutPlatform: localStorage.getItem("scoutPlatform") || "all",
  previewCandidateId: "",
  livePreviewStreamerId: "",
...
function mainAgent101Url() {
  const url = new URL("/", window.location.origin);
  url.searchParams.set("agent101", "1");
  url.searchParams.set("from", "clipping-office");
...
function defaultBuilderEditState() {
  return {
    aspectRatio: "9:16",
    transform: { scale: 112, x: 0, y: 0, rotate: 0 },
    reframe: { enabled: true, target: "Smart AI", sensitivity: 72, smoothness: 82, deadZone: 12, prediction: true, showPath: false },
    background: { mode: "blur", strength: 62, tint: "cyan" },
    autoZoom: { enabled: true, intensity: 58, triggers: "reaction, kill, clutch, loud spike" },
    captions: { enabled: true, style: "Gaming Bold", wordByWord: true, emphasis: true },
    sound: { enhance: true, normalize: true, duckMusic: true },
    timeline: { zoom: 68, snap: true, ripple: false, playhead: 35 },
    tracks: {
      video: { hidden: false, locked: false, solo: false },
      audio: { hidden: false, locked: false, solo: false },
      captions: { hidden: false, locked: false, solo: false },
      stickers: { hidden: false, locked: false, solo: false },
      effects: { hidden: false, locked: false, solo: false },
      zoom: { hidden: false, locked: false, solo: false },
      motion: { hidden: false, locked: false, solo: false },
      safezone: { hidden: false, locked: true, solo: false }
    },
    layers: {
      captions: ["THIS", "SHOT", "IS", "INSANE"],
      stickers: ["!", "NO WAY!"],
      effects: ["Zoom In", "Shake", "Flash"],
      motion: ["Follow Subject"],
      markers: []
    }
  };
}

function mergeBuilderEditState(saved = {}) {
  const base = defaultBuilderEditState();
...
function loadBuilderEditState() {
  try {
    const saved = JSON.parse(localStorage.getItem("builderEditStateV2") || "{}");
    return mergeBuilderEditState(saved);
  } catch {
    return defaultBuilderEditState();
  }
}

function builderEditState(project = state.studio?.project || {}) {
  const projectId = project?.id || "local";
  if (!state.builderEdit || state.builderEditProjectId !== projectId) {
    const saved = project?.editorState && typeof project.editorState === "object"
      ? project.editorState
      : loadBuilderEditState();
    state.builderEdit = mergeBuilderEditState(saved);
    state.builderEditProjectId = projectId;
  }
  return state.builderEdit;
}

function saveBuilderEditState() {
  localStorage.setItem("builderEditStateV2", JSON.stringify(builderEditState()));
}

function pushBuilderHistory() {
  state.builderHistory.push(JSON.stringify(builderEditState()));
  state.builderHistory = state.builderHistory.slice(-40);
  state.builderFuture = [];
}

function undoBuilderEdit() {
  if (!state.builderHistory.length) return false;
  state.builderFuture.push(JSON.stringify(builderEditState()));
  state.builderEdit = mergeBuilderEditState(JSON.parse(state.builderHistory.pop()));
  saveBuilderEditState();
  return true;
}

function redoBuilderEdit() {
  if (!state.builderFuture.length) return false;
  state.builderHistory.push(JSON.stringify(builderEditState()));
  state.builderEdit = mergeBuilderEditState(JSON.parse(state.builderFuture.pop()));
  saveBuilderEditState();
  return true;
}

function builderValueAt(path) {
  return String(path || "").split(".").reduce((value, key) => value?.[key], builderEditState());
}

function setBuilderValue(path, value) {
  const keys = String(path || "").split(".").filter(Boolean);
  if (!keys.length) return;
  let cursor = builderEditState();
...
function coerceBuilderInput(input) {
  if (input.type === "checkbox") return Boolean(input.checked);
  if (input.type === "range" || input.type === "number") return Number(input.value);
  return input.value;
}

function updateBuilderSetting(path, value) {
  pushBuilderHistory();
  setBuilderValue(path, value);
  saveBuilderEditState();
}

function setBuilderTool(tool) {
  state.builderTool = tool || "ai";
  localStorage.setItem("builderTool", state.builderTool);
  render();
}

function setBuilderInspectorTab(tab) {
  state.builderInspectorTab = tab || "inspector";
  localStorage.setItem("builderInspectorTab", state.builderInspectorTab);
  render();
}

function applyBuilderPreset(preset) {
  pushBuilderHistory();
  const edit = builderEditState();
  const presets = {
    "tiktok-gaming": {
      aspectRatio: "9:16",
      background: { mode: "blur", strength: 64, tint: "cyan" },
      reframe: { enabled: true, target: "Crosshair / player", sensitivity: 78, smoothness: 74 },
      autoZoom: { enabled: true, intensity: 68 },
      captions: { enabled: true, style: "Gaming Bold", wordByWord: true }
    },
    "podcast-reaction": {
      aspectRatio: "9:16",
      background: { mode: "soft gradient", strength: 38, tint: "violet" },
      reframe: { enabled: true, target: "Face", sensitivity: 64, smoothness: 86 },
      autoZoom: { enabled: true, intensity: 48 },
      captions: { enabled: true, style: "Clean Creator", wordByWord: true }
    },
    "clean-export": {
      aspectRatio: "9:16",
      background: { mode: "dark", strength: 30, tint: "neutral" },
      reframe: { enabled: true, target: "Largest Motion", sensitivity: 60, smoothness: 90 },
      autoZoom: { enabled: false, intensity: 0 },
      captions: { enabled: false, style: "None", wordByWord: false }
    }
  };
  state.builderEdit = mergeBuilderEditState({ ...edit, ...(presets[preset] || {}) });
  saveBuilderEditState();
  toast("Editor preset applied", "good");
  render();
}

async function syncBuilderEditStateToProject() {
  const projectId = state.studio?.project?.id;
  saveBuilderEditState();
  if (!projectId) {
    toast("Edit settings saved locally", "good");
    return;
  }
  await api(`/api/clip-projects/${encodeURIComponent(projectId)}`, {
    method: "PATCH",
    body: JSON.stringify({
      candidateId: state.selectedCandidateId || state.studio?.project?.selectedCandidateId || "",
      editorState: builderEditState()
    })
  });
  toast("Edit settings saved to the Clip Project", "good");
  await refresh();
}

function addBuilderLayer(type, label) {
  pushBuilderHistory();
  const edit = builderEditState();
  edit.layers[type] ||= [];
  edit.layers[type].push(label);
  saveBuilderEditState();
  toast(`${label} added to ${type}`, "good");
  render();
}

function addBuilderMarker(label) {
  const video = document.querySelector("[data-studio-video]");
  const candidate = studioSelectedCandidate(builderCandidateList(state.studio || {}));
  const current = video ? Number(video.currentTime || 0) : candidateStartSeconds(candidate);
  pushBuilderHistory();
  const edit = builderEditState();
  edit.layers.markers ||= [];
  edit.layers.markers.push({ label, time: Math.max(0, current), id: `marker_${Date.now()}` });
  saveBuilderEditState();
  toast(`${label} marker added at ${formatClock(current)}`, "good");
  render();
}

async function runBuilderCommand(command) {
  const candidate = studioSelectedCandidate(builderCandidateList(state.studio || {}));
  if (command === "undo") {
    toast(undoBuilderEdit() ? "Undo applied" : "Nothing to undo", "info");
    render();
    return;
  }
  if (command === "redo") {
    toast(redoBuilderEdit() ? "Redo applied" : "Nothing to redo", "info");
    render();
    return;
  }
  if (command === "save") {
    await syncBuilderEditStateToProject();
    return;
  }
  if (command === "export" || command === "render-draft") {
    await syncBuilderEditStateToProject();
    await runStudioAction("render-draft");
    return;
  }
  if (command === "ai-auto-edit") {
    applyBuilderPreset(candidate?.category?.toLowerCase?.().includes("podcast") ? "podcast-reaction" : "tiktok-gaming");
    return;
  }
  if (command === "split") {
    addBuilderMarker("Split");
    return;
  }
  if (["mark-start", "mark-end", "capture-frame"].includes(command)) {
    await runStudioAction(command);
    return;
  }
  if (command === "create-package") {
    if (!candidate?.id) return toast("Select a clip first.", "bad");
    await packageCandidate(candidate.id);
    return;
  }
  if (command === "capcut") {
    await createCapCut();
  ...
  if (command === "captions") {
    updateBuilderSetting("captions.enabled", true);
    if (selectedClipPackage(candidate)?.id) await createCaptions();
    else toast("Caption layer enabled. Create a package before writing caption files.", "info");
    render();
    return;
  }
  if (command === "add-text") return addBuilderLayer("captions", "New title");
  if (command === "add-sticker") return addBuilderLayer("stickers", "Sticker");
  if (command === "add-effect") return addBuilderLayer("effects", "Effect");
  if (command === "add-motion") return addBuilderLayer("motion", "Motion keyframe");
  if (command === "delete-layer") {
    pushBuilderHistory();
    const edit = builderEditState();
    const list = edit.layers.effects?.length ? edit.layers.effects : edit.layers.stickers?.length ? edit.layers.stickers : edit.layers.captions;
    if (list?.length) list.pop();
    saveBuilderEditState();
    toast("Selected timeline layer removed", "good");
    render();
    return;
  }
  if (command === "duplicate-layer") {
    pushBuilderHistory();
    const edit = builderEditState();
    const list = edit.layers.effects?.length ? edit.layers.effects : edit.layers.stickers?.length ? edit.layers.stickers : edit.layers.captions;
    if (list?.length) list.push(`${list.at(-1)} copy`);
    saveBuilderEditState();
  ...
  if (command?.startsWith("preset:")) {
    applyBuilderPreset(command.split(":")[1]);
    return;
  }
  if (command?.startsWith("toggle-track:")) {
    const [, track, key] = command.split(":");
    const path = `tracks.${track}.${key}`;
    updateBuilderSetting(path, !builderValueAt(path));
    render();
    return;
  }
}
...
function isPracticeSource(source) {
  return Boolean(
    ...
    || /demo|practice|synthetic/i.test(String(source?.id || ""))
    || /demo|practice|synthetic/i.test(String(source?.sourceType || ""))
  );
}
...
function realCandidates() {
  return state.candidates.filter((candidate) => !isPracticeCandidate(candidate) && !candidateInBuilder(candidate) && candidateIsRadarReady(candidate));
}

function practiceCandidates() {
  return state.candidates.filter((candidate) => isPracticeCandidate(candidate) && !candidateInBuilder(candidate));
}

function candidateIsRadarReady(candidate) {
  if (!candidate) return false;
  if (isPracticeCandidate(candidate)) return false;
  return candidatePlaybackState(candidate).playable;
}

function candidateInBuilder(candidate) {
  return Boolean(
    candidate?.builderDraft
    || candidate?.movedToBuilderAt
    || candidate?.status === "in_builder"
    || (state.packages || []).some((clipPackage) => clipPackage.candidateId === candidate?.id)
  );
}
...
function watchCapacityValue() {
  const configured = Number(
    state.config?.maxWatchedStreamers
    || state.config?.streamWatchCapacity?.limit
    || state.config?.maxWatchers
    || 1
  );
  const limit = Number.isFinite(configured) ? Math.max(1, Math.min(50, Math.floor(configured))) : 1;
  const watching = countWhere(realStreamers(), (streamer) => streamer.monitorEnabled && isPermissionReady(streamer));
  return {
    watching,
    limit,
...
function activeWatchSessionForStreamer(streamerId) {
  return (state.activeWatchSessions || []).find((session) => session.streamerId === streamerId) || null;
}

function savedRadarCandidatesForStreamer(streamerId) {
  return state.candidates.filter((candidate) =>
    candidate.streamerId === streamerId
    && !candidateInBuilder(candidate)
    && candidatePlaybackState(candidate).playable
  );
}

function pendingWatchWindowsForStreamer(streamerId) {
  return (state.activeWatchSessions || [])
    .filter((session) => session.streamerId === streamerId)
    .flatMap((session) => (session.recordingWindows || []).map((window) => ({ ...window, session })))
    .filter((window) => !window.candidateId && window.status !== "source_ready");
}

function clipQualityForStreamer(streamer) {
  const candidates = state.candidates.filter((candidate) => candidate.streamerId === streamer?.id);
  const savedCandidates = savedRadarCandidatesForStreamer(streamer?.id);
  const pendingWindows = pendingWatchWindowsForStreamer(streamer?.id);
  const bestScore = candidates.reduce((max, candidate) => Math.max(max, normalizedNumber(candidate.score)), 0);
  const bestSavedScore = savedCandidates.reduce((max, candidate) => Math.max(max, normalizedNumber(candidate.score)), 0);
  const viewerCount = normalizedNumber(streamer?.liveViewerCount);
  const checkedAt = streamer?.lastCheckedAt ? new Date(streamer.lastCheckedAt).getTime() : 0;
  const minutesSinceCheck = checkedAt ? (Date.now() - checkedAt) / 60000 : Infinity;
  const checkedRecently = Number.isFinite(minutesSinceCheck) && minutesSinceCheck <= 15;
  const live = isStreamerLive(streamer);
  const session = activeWatchSessionForStreamer(streamer?.id);
  const watchedSeconds = Number(session?.analyzedSeconds || 0);
  const windowProgress = watchedSeconds ? (watchedSeconds % 30) / 30 : ((Date.now() / 1000) % 30) / 30;
  const captureBlocked = session?.captureStatus === "blocked" || state.media?.recorder?.ready === false;

  let score = 0;
  if (live) score += 42;
  if (checkedRecently) score += 8;
  if (streamer?.monitorEnabled) score += 8;
  if (session) score += 12;
  if (viewerCount > 0) score += Math.min(28, Math.round(Math.log10(viewerCount + 1) * 8));
  if (savedCandidates.length) score += Math.min(18, 6 + savedCandidates.length * 3);
  if (session && live) score += Math.round(windowProgress * 4);
  if (bestSavedScore) score = Math.max(score, Math.min(100, Math.round(bestSavedScore * 0.82 + 10)));
  score = Math.max(0, Math.min(100, Math.round(score)));

  let label = "Clip quality";
  let detail = "Waiting for a live signal";
  let nextAction = "";
  let savedCount = savedCandidates.length;
  if (savedCandidates.length) {
    label = bestSavedScore >= 80 ? "Saved clip ready" : "Saved clip found";
    detail = `${savedCandidates.length} saved clip${savedCandidates.length === 1 ? "" : "s"} in Radar. ${bestSavedScore ? `Best score ${bestSavedScore}/100.` : "Open Radar to score it."}`;
    nextAction = "Open Radar";
  } else if (session?.captureStatus === "capturing") {
    label = "Saving clip buffer";
    detail = session.captureMessage || "A 30s local buffer is being saved. Radar will show it after verification.";
    nextAction = "Saving";
  } else if (captureBlocked) {
    label = "Recorder blocked";
    detail = session?.captureMessage || state.media?.recorder?.message || "Live signal is available, but no local video buffer can be saved yet.";
    nextAction = "Fix recorder";
  } else if (pendingWindows.length) {
    label = "Live window pending";
    detail = `${pendingWindows.length} watch window${pendingWindows.length === 1 ? "" : "s"} need a saved 30s buffer before Radar can show playback.`;
    nextAction = "Save 30s";
  } else if (session && live) {
    label = score >= 70 ? "Strong live signal" : "Scanning live";
    detail = `Agent is watching ${Math.round(watchedSeconds || windowProgress * 30)}s of live signal. Save a 30s buffer before this becomes a Radar clip.`;
    nextAction = "Save 30s";
  } else if (live) {
    label = score >= 70 ? "Strong live signal" : "Live signal";
    detail = streamer?.monitorEnabled ? "Live and ready for watcher analysis." : "Live now. Approval and monitoring are separate from clip quality.";
    nextAction = streamer?.monitorEnabled ? "Start watcher" : "Approve";
  } else if (isStreamerConfirmedOffline(streamer)) {
    label = "Offline signal";
    detail = "Streamer is offline, so clip quality is low until they go live.";
    nextAction = "Wait for live";
  } else if (streamer?.liveStatus === "api_not_configured") {
    label = "No API signal";
    detail = "Provider is not configured, so quality cannot be measured.";
    nextAction = "Connect API";
  } else if (!checkedRecently) {
    label = "Signal stale";
    detail = "Run check to refresh live quality.";
    nextAction = "Refresh check";
  }

  const tone = score >= 70 ? "good" : score >= 38 ? "warn" : "bad";
  const gate = isPermissionReady(streamer) ? "Approved" : "Approval gate";
  return { score, tone, label, detail, gate, candidates: candidates.length, savedCount, pendingWindows: pendingWindows.length, bestScore, bestSavedScore, nextAction, captureBlocked };
}

function renderClipReadinessMeter(streamer, options = {}) {
  const readiness = clipQualityForStreamer(streamer);
  const compact = options.compact ? " compact" : "";
  return `
    <div class="clip-readiness ${esc(readiness.tone)}${compact}" title="${esc(readiness.detail)}">
      <div class="clip-readiness-head">
        <b>${esc(readiness.label)}</b>
        <span>${esc(readiness.score)}%</span>
      </div>
      <i style="--quality:${esc(readiness.score)}"><em></em></i>
      ${options.compact ? "" : `<small><strong>${esc(readiness.gate)}:</strong> ${esc(readiness.detail)}</small>`}
    </div>
  `;
}
...
function candidateSource(candidate) {
  ...
  return isPracticeSource(source) ? null : source;
}

function candidatePlaybackState(candidate) {
  const source = candidateSource(candidate);
  const playable = Boolean(
    (source?.playable && source?.playbackUrl)
    || (candidate?.mediaPlayable && (candidate?.playbackUrl || source?.playbackUrl))
  );
  const sourcePending = candidate?.sourceType === "live_recording_window" && !playable;
  const bufferStatus = candidate?.bufferStatus || (playable ? "playable" : "source_pending");
  return {
    source,
    playable,
    sourcePending,
    bufferStatus,
    label: playable ? "Playable clip" : sourcePending ? "Needs video" : "No video source",
    detail: playable
      ? "A local playable source is attached."
      : sourcePending
        ? "Capture or upload source media before this becomes a Builder clip."
      : "No local playable source is attached to this candidate."
  };
}

function candidateNeedsWatcherCleanup(candidate) {
  const playback = candidatePlaybackState(candidate);
  return Boolean(candidate?.sourceType === "live_recording_window" && playback.sourcePending);
}

function candidateDisplayTitle(candidate) {
  if (!candidate) return "Untitled clip";
  if (candidate.sourceType === "live_recording_window") {
    const streamer = state.streamers.find((item) => item.id === candidate.streamerId);
    const index = Number(candidate.recordingWindowIndex);
    const label = Number.isFinite(index) ? `Window ${index + 1}` : "Live window";
    return `${label}: ${streamer?.displayName || candidate.streamerName || "Live stream"}`;
  }
  return candidate.title || "Untitled clip";
}

function streamerSourceUrl(streamer) {
  if (!streamer) return "";
...
function previewFrame({
  ...
  return `
    <div class="media-frame ${esc(className)} thumb-${index % 5}">
      ...
      ${label ? `<span class="media-badge">${esc(label)}</span>` : ""}
      ${candidateId
        ? `<button class="media-play ${playable ? "" : "details"}" type="button" data-preview-candidate="${esc(candidateId)}" aria-label="${esc(actionLabel)} ${mediaTitle(title)}">${esc(actionLabel)}</button>`
        : `<span class="media-play locked" aria-hidden="true">${esc(actionLabel || "Preview")}</span>`}
...
function renderSidebarOps() {
  $("#sidebar-ops").innerHTML = `
    <a class="sidebar-agent-chat-card" href="${esc(mainAgent101Url())}" aria-label="Open Agent 101 in Argentum">
      <span class="agent-chat-mark">A</span>
      <div>
        <strong>Open Agent 101</strong>
        <small>Main Argentum command center</small>
      </div>
...
async function api(path, options = {}) {
  ...
  if (!contentType.includes("application/json")) {
    ...
    const redirectedToLogin = response.redirected && /\/login(?:$|[?#])/.test(new URL(response.url, window.location.origin).pathname);
    throw new Error(redirectedToLogin ? "Session expired. Sign in to Argentum OS again." : `Expected JSON from ${path}; received ${contentType || "unknown content"}`);
  }
...
const coreEndpoints = [
  ["health", "/api/health", {}],
  ["config", "/api/config", {}],
  ["openai", "/api/openai/status", { configured: false }],
  ["twitch", "/api/twitch/status", { configured: false }],
  ["kick", "/api/kick/status", { configured: false }],
  ["browser", "/api/browser/profile", {}],
  ["capcut", "/api/capcut/status", {}],
  ["capcutSessions", "/api/capcut/sessions", { sessions: [] }],
  ["media", "/api/media/status", {}],
  ["mediaSources", "/api/media/sources", { sources: [] }],
  ["integrations", "/api/integrations/status", {}],
  ["readiness", "/api/readiness/audit", {}],
  ["handoffs", "/api/handoffs", { handoffs: [] }],
  ["smoke", "/api/system/smoke-test", { latest: null }],
  ["agent101Sessions", "/api/agent101/sessions", { sessions: [] }],
  ["studio", "/api/clipping-office/project", {}],
  ["watchSessions", "/api/watch-sessions/active", { sessions: [], events: [] }],
  ["streamers", "/api/twitch/streamers", { streamers: [] }],
  ["candidates", "/api/clips/candidates", { candidates: [] }],
  ["packages", "/api/clips/packages", { packages: [] }],
  ["posts", "/api/posts/queue", { drafts: [], dailyLimit: 0 }],
  ["approvals", "/api/human-gate/approvals", { approvals: [] }],
  ["artifacts", "/api/artifacts", { artifacts: [] }],
  ["logs", "/api/logs", { logs: [] }]
];

async function loadCoreEndpoint([key, path, fallback]) {
  try {
    return [key, await api(path), null];
  } catch (error) {
    return [key, fallback, { key, path, message: error.message || "Request failed" }];
  }
}
...
async function loadCore() {
  const results = await Promise.all(coreEndpoints.map(loadCoreEndpoint));
  const loaded = Object.fromEntries(results.map(([key, value]) => [key, value]));
  const failures = results.map(([, , failure]) => failure).filter(Boolean);
  const { health, config, openai, twitch, kick, browser, capcut, capcutSessions, media, mediaSources, integrations, readiness, handoffs, smoke, agent101Sessions, studio, watchSessions, streamers, candidates, packages, posts, approvals, artifacts, logs } = loaded;
  Object.assign(state, {
    health,
    config,
    openai,
    ...
    browser,
    capcut,
    capcutAgentSessions: capcutSessions.sessions || [],
    media,
    ...
    agent101Sessions: agent101Sessions.sessions || [],
    studio,
  ...
  if (failures.length) updateApiDegradedStatus(failures);
}

function syncSelectionsAfterLoad() {
  const candidateIds = new Set((state.candidates || []).map((candidate) => candidate.id));
  state.selectedCandidateIds = state.selectedCandidateIds.filter((id) => candidateIds.has(id));
  if (state.selectedCandidateId && !candidateIds.has(state.selectedCandidateId)) {
    state.selectedCandidateId = state.candidates[0]?.id || "";
    if (state.selectedCandidateId) localStorage.setItem("selectedCandidateId", state.selectedCandidateId);
    else localStorage.removeItem("selectedCandidateId");
  }
  if (state.previewCandidateId && !candidateIds.has(state.previewCandidateId)) {
    state.previewCandidateId = "";
  }

  const streamerIds = new Set((state.streamers || []).map((streamer) => streamer.id));
  state.selectedStreamerIds = state.selectedStreamerIds.filter((id) => streamerIds.has(id));
  if (state.selectedStreamerId && !streamerIds.has(state.selectedStreamerId)) {
    state.selectedStreamerId = state.streamers[0]?.id || "";
    if (state.selectedStreamerId) localStorage.setItem("selectedStreamerId", state.selectedStreamerId);
    else localStorage.removeItem("selectedStreamerId");
  }
}

function setRefreshBusy(isBusy) {
  state.refreshBusy = Boolean(isBusy);
...
function updateRefreshControl() {
  ...
  if (!button) return;
  button.disabled = state.refreshBusy;
  button.classList.toggle("is-busy", state.refreshBusy);
  button.textContent = state.refreshBusy ? "Refreshing..." : "Refresh";
}
...
function connectWatchSessionEvents() {
  ...
  source.addEventListener("watch_event", (event) => {
    try {
      ...
      if (["recording_window_created", "recording_window_waiting_for_source", "source_capture_started", "source_capture_completed", "source_capture_blocked", "candidate_review", "candidate_accepted", "candidate_rejected", "render_completed", "source_capability_degraded", "session_stopped"].includes(parsed.type)) {
...
function updateApiDegradedStatus(failures = []) {
  const apiStatus = $("#api-status");
  if (!apiStatus || !failures.length) return;
  apiStatus.className = "pill warn";
  apiStatus.textContent = `${failures.length} API issue${failures.length === 1 ? "" : "s"}`;
  apiStatus.title = failures.map((failure) => `${failure.path}: ${failure.message}`).join("\n");
}
...
function navCount(key) {
  ...
  if (key === "agentStudio") return state.agent101Sessions.length || "";
  if (key === "candidates") return radarCandidateScope().visibleCandidates.length || "";
  if (key === "builder") return builderQueueCandidates().length || "";
...
function navIcon(icon) {
  const icons = {
    ...
    agent: `<svg viewBox="0 0 24 24"><path d="M12 3 19 7v10l-7 4-7-4V7Z"/><path d="M8 12h8"/><path d="M9 16h6"/><circle cx="9" cy="9" r="1"/><circle cx="15" cy="9" r="1"/></svg>`,
...
function setView(id) {
  if (id === "browser") id = "builder";
  if (id === "agent101") {
    window.location.assign(mainAgent101Url());
    return;
  }
  state.view = id;
  syncTopbarMode();
...
function syncTopbarMode() {
  document.querySelector(".topbar")?.classList.toggle("radar-focus", state.view === "radar");
}
...
function render() {
  ...
  try {
    renderer();
    view.insertAdjacentHTML("beforeend", renderLiveMonitorModal());
...
function renderDashboard() {
  ...
  const capacity = watchCapacityValue();
  const watched = capacity.watching;
  ...
  view.innerHTML = `
    ...
    <div class="metric-strip">
      ${metric("Watched Streams", watched, `${capacity.watching}/${capacity.limit} watch slots`, "CAM", capacity.atLimit ? "warn" : "good")}
    ...
    <div class="dashboard-main">
      <section class="panel live-desk">
        <div class="toolbar">
          <div>
            <h2>Live Desk</h2>
            <p class="muted">Realtime stream monitoring · ${capacity.watching}/${capacity.limit} watch slots</p>
          </div>
          <div class="actions">
            <button class="primary" data-action="run-watch">Run Watch Cycle</button>
    ...
    <section class="panel quick-command">
      ...
      <div class="quick-grid">
        ...
        <button data-nav-jump="builder"><span>C</span><b>CapCut Agent</b><small>Edit from Clip Builder</small></button>
    ...
    <section class="panel agent-run-panel">
      <div class="agent-run-head">
        <div>
          ...
          <p class="muted">${esc(run.summary || "Run production-safe clipping tasks without publishing, spending, or changing accounts.")}</p>
        </div>
...
function activeAgent101SessionSummary() {
  return state.agent101Sessions.find((session) => session.sessionId === state.activeAgent101SessionId) || state.agent101Sessions[0] || null;
}

function activeAgent101Runs() {
  return state.agent101Session?.runs || [];
}

function isVerifiedRenderedArtifact(artifact) {
  return Boolean(
    artifact?.type === "rendered_clip"
    && artifact.content?.sha256
    && ["passed", "verified"].includes(artifact.content?.probeStatus)
  );
}

function renderedArtifactForCandidate(candidate, fallbackArtifact = null) {
  if (!candidate) return null;
  if (isVerifiedRenderedArtifact(fallbackArtifact)) return fallbackArtifact;
  const packageRow = (state.packages || []).find((item) => item.candidateId === candidate.id);
  const artifactIds = [candidate.renderedArtifactId, packageRow?.renderedArtifactId].filter(Boolean);
  return (state.artifacts || []).find((artifact) => artifactIds.includes(artifact.id) && isVerifiedRenderedArtifact(artifact)) || null;
}

function capcutReadyCandidates() {
  return (state.candidates || []).filter((candidate) => renderedArtifactForCandidate(candidate));
}

function activeCapcutSessionForCandidate(candidate) {
  return (
    state.capcutAgentSessions.find((item) => item.sessionId === state.activeCapcutAgentSessionId && (!candidate?.id || item.clipId === candidate.id))
    || state.capcutAgentSessions.find((item) => candidate?.id && item.clipId === candidate.id)
    || state.capcutAgentSessions.find((item) => item.sessionId === state.activeCapcutAgentSessionId)
    || state.capcutAgentSessions[0]
    || null
  );
}

function renderCapcutAgentEventList(activeSession, limit = 5) {
  if (!activeSession) return `<div class="empty-inline">Start the CapCut Agent to see live progress.</div>`;
  return `
    <div class="capcut-session-meta">
      ${badge(activeSession.status || "queued", activeSession.status === "error" ? "bad" : activeSession.status === "completed" ? "good" : "info")}
      ${activeSession.loginApprovalId ? badge("Login gate", "warn") : ""}
      ${activeSession.exportApprovalId ? badge("Export gate", "warn") : ""}
    </div>
    ${(activeSession.events || []).slice(-limit).map((event) => `
      <div class="agent101-event ${esc(event.status || "")}">
        <b>${esc((event.phase || event.event || "step").replaceAll("_", " "))}</b>
        <span>${esc(event.message || event.status || "")}</span>
      </div>
    `).join("") || `<div class="empty-inline">Agent run is queued.</div>`}
  `;
}

function renderAgent101CapcutPanel() {
  const candidates = capcutReadyCandidates();
  const activeSession = activeCapcutSessionForCandidate();
  return `
    <section class="panel agent101-capcut-panel">
      <div class="toolbar">
        <div>
          <span class="eyebrow">CapCut Agent</span>
          <h2>Supervised edit automation</h2>
          <p class="muted">Uses only verified rendered MP4s. Login confirmation and export/download stay in Human Gate.</p>
        </div>
        ${badge(state.capcut?.agentReady ? "Supervised ready" : "Needs CapCut login", state.capcut?.agentReady ? "good" : "warn")}
      </div>
      <div class="agent101-capcut-grid">
        <form id="agent101-capcut-form" class="agent101-capcut-form">
          <label>
            <span>Verified clip</span>
            <select name="clip_id" ${state.capcutAgentBusy || !candidates.length ? "disabled" : ""}>
              ${candidates.length ? candidates.map((candidate) => `
                <option value="${esc(candidate.id)}">${esc(candidate.title || candidate.id)}</option>
              `).join("") : `<option value="">Render a verified clip first</option>`}
            </select>
          </label>
          <div class="agent101-capcut-controls">
            <label><span>Format</span><select name="aspect_ratio"><option value="9:16">9:16 TikTok/Reels</option><option value="1:1">1:1 Square</option><option value="16:9">16:9 Source</option></select></label>
            <label><span>Platform</span><select name="platform"><option value="tiktok">TikTok</option><option value="instagram_reels">Instagram Reels</option><option value="youtube_shorts">YouTube Shorts</option></select></label>
            <label class="inline-check"><input type="checkbox" name="auto_captions" checked> Auto captions</label>
            <label class="inline-check"><input type="checkbox" name="auto_reframe" checked> Auto reframe</label>
          </div>
          <textarea name="hook_text" rows="2" placeholder="Optional hook text or edit note for Agent 101"></textarea>
          <label class="inline-check"><input type="checkbox" name="practice_confirmed"> I understand practice media is not a real production clip</label>
          <div class="actions-row">
            <button class="primary" type="submit" ${state.capcutAgentBusy || !candidates.length ? "disabled" : ""}>Edit in CapCut</button>
            <button type="button" data-action="refresh">Refresh</button>
          </div>
        </form>
        <div class="agent101-capcut-sessions">
          <h3>Recent CapCut runs</h3>
          ${state.capcutAgentSessions.length ? state.capcutAgentSessions.slice(0, 5).map((session) => `
            <button class="${session.sessionId === activeSession?.sessionId ? "active" : ""}" data-capcut-agent-session="${esc(session.sessionId)}">
              <b>${esc(session.title || "CapCut edit")}</b>
              <span>${esc(session.stage || session.status || "queued")} · ${esc(fmtDate(session.updatedAt || session.createdAt))}</span>
            </button>
          `).join("") : `<div class="empty-inline">No CapCut Agent runs yet.</div>`}
        </div>
        <div class="agent101-capcut-events">
          <h3>${esc(activeSession?.title || "No active run")}</h3>
          ${renderCapcutAgentEventList(activeSession, 8)}
        </div>
      </div>
    </section>
  `;
...
function renderAgent101Studio() {
  window.location.assign(mainAgent101Url());
  view.innerHTML = `
    <section class="panel">
      ${empty("Opening Agent 101 in Argentum OS.")}
    </section>
  `;
}

function renderAgent101Run(run) {
  return `
    <article class="agent101-run-card">
      <div class="agent101-message user"><b>Operator</b><p>${esc(run.userMessage || run.goal || "")}</p></div>
      <div class="agent101-message assistant">
        <b>Agent 101</b>
        <p>${esc(run.response || run.summary || "Working...")}</p>
      </div>
      ${(run.toolCalls || []).length ? `
        <details class="agent101-tool-details" open>
          <summary>${esc(run.toolCalls.length)} tool calls</summary>
          <div class="agent101-tool-list">
            ${(run.toolCalls || []).map((tool) => `
              <section>
                <b>${esc(tool.name)}</b>
                <span>${esc(tool.status || "completed")} · ${esc(tool.durationMs || 0)}ms</span>
                <pre>${esc(JSON.stringify({ input: tool.input, output: tool.output }, null, 2))}</pre>
              </section>
            `).join("")}
          </div>
        </details>
...
function renderAgent101FileModal() {
  const file = state.agent101FileModal;
  if (!file) return "";
  return `
    <div class="agent101-file-modal" role="dialog" aria-modal="true">
      <section>
        <div class="toolbar">
          <h2>${esc(file.path || "Output file")}</h2>
          <button class="icon-button" data-close-agent101-file>×</button>
        </div>
        ${file.binary ? `<p class="muted">Binary file. Open or download it from the output card.</p>` : `<pre>${esc(file.content || "")}</pre>`}
      </section>
    </div>
  `;
}
...
function renderLiveDesk() {
  const allStreamers = realStreamers();
  const streamers = allStreamers.filter((streamer) => streamer.monitorEnabled && isPermissionReady(streamer));
  const capacity = watchCapacityValue();
  if (!streamers.length) {
    return `
      <div class="empty-mission">
        <strong>${allStreamers.length ? "No active watched streamers" : "No real streamers loaded yet"}</strong>
        <p>Add or approve creators, then turn monitoring on. Capacity is ${capacity.watching}/${capacity.limit} watch slots.</p>
        <div class="actions">
          <button class="primary" data-nav-jump="watchlist">${allStreamers.length ? "Open Watchlist" : "Add Real Streamer"}</button>
        </div>
      </div>
    `;
  }
  const sortedStreamers = [...streamers].sort((a, b) => {
    ...
    const monitorDelta = Number(b.monitorEnabled) - Number(a.monitorEnabled);
    if (monitorDelta) return monitorDelta;
    return String(b.lastCheckedAt || "").localeCompare(String(a.lastCheckedAt || ""));
  });
  const densityClass = sortedStreamers.length >= 28 ? "ultra" : sortedStreamers.length >= 12 ? "dense" : "";
  const cards = sortedStreamers.map((streamer, index) => {
    const savedCandidates = savedRadarCandidatesForStreamer(streamer.id);
    const session = activeWatchSessionForStreamer(streamer.id);
    const status = liveStatusMeta(streamer);
    const readiness = clipQualityForStreamer(streamer);
    return `
    <article class="stream-card" data-select-streamer="${esc(streamer.id)}" data-live-preview-streamer="${esc(streamer.id)}">
      ${previewFrame({
        label: streamer.liveStatus === "live" ? "LIVE NOW" : status.label,
        title: streamer.liveTitle || `${streamer.displayName} stream`,
        subtitle: streamer.liveCategory || streamer.notes || "Local monitored source",
        timestamp: "LIVE",
        end: session?.captureStatus === "capturing" ? "Saving" : `${savedCandidates.length} saved`,
        index,
      ...
      ${renderClipReadinessMeter(streamer, { compact: true })}
      <div class="stream-foot">
        <span>${fmtDate(streamer.lastCheckedAt)}</span>
        <span class="spark">${sparkline(index * 7)}</span>
      </div>
      <div class="stream-actions">
        <button type="button" data-live-preview-streamer="${esc(streamer.id)}">Live Monitor</button>
        ${session?.id ? `<button class="primary" type="button" data-capture-session="${esc(session.id)}">${savedCandidates.length ? "Save Another 30s" : "Save 30s to Radar"}</button>` : `<button type="button" data-toggle-monitor="${esc(streamer.id)}">Start Watcher</button>`}
      </div>
    </article>
  `;
  }).join("");
  const addAttrs = capacity.atLimit ? `disabled aria-disabled="true" title="Watch capacity is full. Pause another stream before adding more."` : `data-nav-jump="watchlist"`;
  return `
    <div class="stream-grid-head ${capacity.atLimit ? "warn" : ""}">
      <span><b>${esc(capacity.watching)}</b> watching now</span>
      <span>${esc(capacity.remaining)} open slot${capacity.remaining === 1 ? "" : "s"} · max ${esc(capacity.limit)}</span>
    </div>
    <div class="stream-grid ${densityClass}">
      ${cards}
      <button class="add-stream-card ${capacity.atLimit ? "disabled" : ""}" ${addAttrs}><b>+</b><span>${capacity.atLimit ? "Limit Reached" : "Add Streamer"}</span><small>${capacity.atLimit ? "Pause one to open a slot" : "Monitor a new channel"}</small></button>
    </div>
  `;
}
...
function renderTopCandidates() {
  ...
  const candidates = real.slice(0, 5);
  if (!candidates.length) return empty("No saved clip candidates yet. Add an approved streamer and run a watch cycle.");
...
function renderWatchlist() {
  ...
  const capacity = watchCapacityValue();
  const monitoringCount = capacity.watching;
  ...
  const activeFilter = ["all", "live", "monitoring", "offline", "pending"].includes(state.watchlistFilter) ? state.watchlistFilter : "all";
  const visibleStreamers = filterWatchlistStreamers(streamers, activeFilter);
  view.innerHTML = `
    <section class="watchlist-page">
      <div class="watchlist-tabs">
        ${watchlistTab("all", "All Streamers", streamers.length, activeFilter)}
        ${watchlistTab("live", "Live Now", liveCount, activeFilter)}
        ${watchlistTab("monitoring", "Monitoring", `${monitoringCount}/${capacity.limit}`, activeFilter)}
        ${watchlistTab("offline", "Offline", offlineCount, activeFilter)}
        ${watchlistTab("pending", "Pending", pendingCount, activeFilter)}
      </div>
      ...
      <div class="watchlist-stats">
        ...
        ${watchStat("Monitoring", `${monitoringCount}/${capacity.limit}`, `${capacity.remaining} open slots`, "EYE", capacity.atLimit ? "warn" : "info")}
      ...
      <div class="watch-capacity-banner ${capacity.atLimit ? "warn" : ""}">
        <strong>Watch capacity</strong>
        <span>${capacity.watching}/${capacity.limit} active monitors. ${capacity.remaining ? `${capacity.remaining} slots available for more streamers.` : "Pause a monitored streamer before adding another one."}</span>
      </div>
      ...
      <div class="watchlist-shell">
        <section class="panel streamer-directory">
          ...
          ${renderStreamerBulkActions(visibleStreamers)}
          ${visibleStreamers.length ? renderStreamerTable(true, visibleStreamers) : empty(watchlistEmptyMessage(activeFilter))}
        </section>
...
function watchlistTab(filter, label, count, activeFilter) {
  return `<button class="${activeFilter === filter ? "active" : ""}" data-watchlist-filter="${esc(filter)}">${esc(label)} <b>${esc(count)}</b></button>`;
}

function filterWatchlistStreamers(streamers, filter) {
  if (filter === "live") return streamers.filter(isStreamerLive);
  if (filter === "monitoring") return streamers.filter((streamer) => streamer.monitorEnabled && isPermissionReady(streamer));
  if (filter === "offline") return streamers.filter(isStreamerConfirmedOffline);
  if (filter === "pending") return streamers.filter((streamer) => streamer.permissionStatus === "pending");
  return streamers;
}

function watchlistEmptyMessage(filter) {
  if (filter === "live") return "No live streamers in this filtered view. Run a check or scout streamers with provider APIs connected.";
  if (filter === "monitoring") return "No approved streamers are currently being monitored.";
  if (filter === "offline") return "No streamer is confirmed offline right now.";
  if (filter === "pending") return "No streamers are waiting for permission review.";
  return "No real streamers added. Use Add Streamer for production records.";
}

function renderStreamerBulkActions(streamers) {
  const selectedIds = new Set(state.selectedStreamerIds);
  const selectedCount = streamers.filter((streamer) => selectedIds.has(streamer.id)).length;
  return `
    <div class="streamer-bulk-actions ${selectedCount ? "active" : ""}">
      <span>${selectedCount ? `${selectedCount} selected` : "Select rows to remove or review"}</span>
      <button class="danger" data-action="remove-selected-streamers" ${selectedCount ? "" : "disabled"}>Remove selected</button>
    </div>
  `;
}
      ...
      <table class="watch-table">
        <thead>
          <tr>
            ...
            <th>Live Status</th>
            <th>Last Checked</th>
            <th>Clip Readiness</th>
            ${editable ? "<th>Actions</th>" : ""}
          </tr>
        </thead>
        <tbody>
          ${sortedStreamers.map((streamer, index) => {
            const monitorDisabled = !streamer.monitorEnabled && isPermissionReady(streamer) && capacity.atLimit;
            const primaryAction = streamerRowPrimaryAction(streamer);
            return `
            <tr class="${selectedStreamer()?.id === streamer.id ? "selected" : ""}" data-select-streamer="${streamer.id}">
              <td><input class="streamer-select-box" type="checkbox" data-streamer-check="${streamer.id}" ${selectedIds.has(streamer.id) ? "checked" : ""} aria-label="Select ${esc(streamer.displayName)}"></td>
              <td>
                <div class="streamer-cell">
                  <span class="creator-avatar avatar-${index % 6}">${esc(initials(streamer.displayName))}</span>
                  <div>
                    <strong>${esc(streamer.displayName)} <em>${esc(streamerTrustLabel(streamer))}</em></strong>
              ...
              ${editable ? `<td><div class="actions">
                <button data-toggle-monitor="${streamer.id}" ${monitorDisabled ? `disabled aria-disabled="true" title="Watch capacity is full. Pause another streamer first."` : ""}>${esc(primaryAction)}</button>
                <button data-check-streamer="${streamer.id}">Check</button>
                ${streamer.permissionStatus === "approved" ? "" : `<button data-approve-streamer="${streamer.id}">Approve</button>`}
                <button class="danger" data-delete-streamer="${streamer.id}">Remove</button>
              </div></td>` : ""}
            </tr>
          `;
          }).join("")}
...
function streamerRowPrimaryAction(streamer) {
  if (streamer.monitorEnabled) return "Pause";
  if (streamer.permissionStatus !== "approved" && isStreamerLive(streamer)) return "Approve + Watch";
  if (streamer.permissionStatus === "approved" && isStreamerLive(streamer)) return "Start 30s Watch";
  return "Monitor";
}

function streamerTrustLabel(streamer) {
  if (isPracticeStreamer(streamer)) return "practice";
  if (streamer?.liveStatus === "live") return "live verified";
  if (streamer?.liveStatus === "api_not_configured") return "api needed";
  if (streamer?.permissionStatus === "pending") return "review";
  if (streamer?.permissionStatus === "blocked") return "blocked";
  if (streamer?.lastCheckedAt) return "checked";
  return "unverified";
}
...
function renderStreamerScout() {
  ...
  const blocked = !hasRecommendations && (state.recommendationsMeta?.providerBlocked || state.recommendationsMeta?.fallbackUsed);
  return `
    <section class="panel streamer-scout-panel">
      <div class="scout-head">
        ...
        <div class="scout-actions">
          <select id="scout-platform" data-scout-platform aria-label="Scout platform">
            <option value="all" ${state.scoutPlatform === "all" ? "selected" : ""}>Kick + Twitch</option>
            <option value="kick" ${state.scoutPlatform === "kick" ? "selected" : ""}>Kick only</option>
            <option value="twitch" ${state.scoutPlatform === "twitch" ? "selected" : ""}>Twitch only</option>
          </select>
      ...
      <div class="scout-recommendations">
        ${hasRecommendations ? state.recommendations.map(renderStreamerRecommendation).join("") : blocked ? renderScoutBlocked() : empty("No scout run yet. Click Scout streamers after provider variables finish deploying.")}
      </div>
    </section>
  `;
}

function renderScoutBlocked() {
  const manual = state.manualReviewRecommendations || [];
  const names = manual.slice(0, 6).map((item) => item.displayName || item.channelId).filter(Boolean);
  return `
    <article class="scout-blocked">
      <div>
        <strong>Verified live scout is waiting on provider access</strong>
        <p>${esc(state.recommendationsMessage || "Agent 101 did not create fake live recommendations. Connect Twitch/Kick API access to rank real streams with real viewer counts.")}</p>
        ${names.length ? `<small>Manual review names only, not live checked: ${esc(names.join(", "))}</small>` : ""}
      </div>
      <div class="scout-blocked-actions">
        <button class="primary" data-nav-jump="integrations">Connect APIs</button>
        <button data-focus-add-streamer>Add channel manually</button>
      </div>
    </article>
  `;
}
...
function renderStreamerRecommendation(item, index) {
  const verifiedLive = Boolean(item.liveVerified && item.liveStatus === "live" && Number(item.viewerCount || 0) > 0);
  const reviewOnly = !verifiedLive || item.canAutoMonitor === false || item.sourceType === "manual_review";
  const liveLabel = verifiedLive ? badge("Live verified", "good") : badge("Live not verified", "warn");
  const actionLabel = reviewOnly ? "Add for review" : "Add live lead";
  const viewerCopy = verifiedLive
    ? `${Number(item.viewerCount || 0).toLocaleString()} live viewers`
    : "Viewers unavailable until provider check";
  return `
    <article class="scout-card">
      ...
      <div class="scout-copy">
        ...
        <small>${esc(item.category || "General")} - ${esc(viewerCopy)} - ${esc(item.source || "Agent 101")}</small>
        <em>${esc(item.reason || "Good candidate for supervised monitoring.")}</em>
      </div>
      <button data-add-recommendation="${index}">${esc(actionLabel)}</button>
    </article>
  `;
}
...
function renderCompactStreamerForm() {
  ...
  return `
    <form id="streamer-form" class="streamer-mini-form">
      <p class="capacity-note ${capacity.atLimit ? "warn" : ""}">Watching ${esc(capacity.watching)}/${esc(capacity.limit)} streamers. ${capacity.remaining ? `${esc(capacity.remaining)} slots open.` : "Capacity is full; add new channels paused."}</p>
      ...
      <select name="monitorEnabled">
        <option value="true" ${capacity.atLimit ? "disabled" : ""}>monitoring on</option>
        <option value="false" ${capacity.atLimit ? "selected" : ""}>monitoring off</option>
      </select>
...
function renderStreamerInspector(streamer) {
  ...
  return `
    ...
    <div class="inspector-section">
      <h3>Performance</h3>
      ${renderClipReadinessMeter(streamer)}
...
function renderRadar() {
  const { visibleCandidates, sorted } = radarCandidateScope();
  const visibleIds = new Set(visibleCandidates.map((candidate) => candidate.id));
  state.selectedCandidateIds = state.selectedCandidateIds.filter((id) => visibleIds.has(id));
  if (state.selectedCandidateId && !visibleIds.has(state.selectedCandidateId)) {
    state.selectedCandidateId = sorted[0]?.id || "";
    if (state.selectedCandidateId) localStorage.setItem("selectedCandidateId", state.selectedCandidateId);
    else localStorage.removeItem("selectedCandidateId");
  }
  ...
  const selectedCandidates = sorted.filter((candidate) => selectedIds.has(candidate.id));
  view.innerHTML = `
    <section class="radar-page">
      <div class="radar-command-bar panel">
        <div>
          <span class="eyebrow">Clip Radar</span>
          <h2>All candidates <b>${visibleCandidates.length}</b></h2>
          <p>Review windows, send keepers to Builder, delete bad ones. Watch controls stay in Stream Watchlist.</p>
        </div>
        <div class="radar-command-actions">
          <button data-action="run-watch">Run Watch Cycle</button>
          <button class="danger" data-action="delete-selected-candidates" ${selectedCount ? "" : "disabled"}>Delete Selected</button>
          <button class="danger" data-action="delete-visible-candidates" ${sorted.length ? "" : "disabled"}>Delete All Visible</button>
        </div>
      </div>

      ${renderRadarCaptureStatus(visibleCandidates.length)}
      ...
      <div class="radar-shell">
        <section class="panel radar-board">
          <div class="radar-filterbar">
            ...
            <select aria-label="Streamer filter"><option>Streamer: All</option>${realStreamers().map((streamer) => `<option>${esc(streamer.displayName)}</option>`).join("")}</select>
          ...
          ${renderRadarSelectionBar(sorted, selectedCandidates)}
          ${sorted.length ? renderRadarTable(sorted) : empty("No saved clip candidates yet. Start an approved watcher or capture source from a live stream.")}
        </section>

        <aside class="panel radar-inspector">
          ${selectedCount > 1 ? renderBulkCandidateInspector(selectedCandidates, sorted.length) : selected ? renderCandidateInspector(selected) : empty("Select a clip candidate")}
        </aside>
      </div>
    </section>
  `;
...
function pendingWatchWindows() {
  return (state.activeWatchSessions || []).flatMap((session) =>
    (session.recordingWindows || []).map((window) => ({ ...window, session }))
  ).filter((window) => !window.candidateId && window.status !== "source_ready");
}

function renderRadarCaptureStatus(visibleCount = 0) {
  if (visibleCount) return "";
  const sessions = state.activeWatchSessions || [];
  const pending = pendingWatchWindows();
  const recorder = state.media?.recorder || {};
  const active = sessions.find((session) => ["queued", "starting", "connecting", "watching", "degraded", "reconnecting"].includes(session.status));
  let title = "No saved clips in Radar yet";
  let detail = "Start an approved live watcher or upload source media in Clip Builder. Radar only shows playable saved clips now.";
  let tone = "info";
  if (active?.captureStatus === "capturing") {
    title = "Saving a local clip buffer";
    detail = active.captureMessage || "The watcher is recording a 30-second local source file. It will appear here after FFprobe verifies it.";
    tone = "good";
  } else if (active?.captureStatus === "blocked" || (active && recorder.ready === false)) {
    title = "Live monitor is running, recorder is blocked";
    detail = active.captureMessage || recorder.message || "Install streamlink or yt-dlp so live windows can become saved clips.";
    tone = "warn";
  } else if (pending.length) {
    title = `${pending.length} watch window${pending.length === 1 ? "" : "s"} waiting for saved video`;
    detail = pending[0]?.message || "The watcher has telemetry, but no playable clip file has been attached yet.";
    tone = "warn";
  } else if (active) {
    title = "Watcher is active";
    detail = active.captureMessage || "Waiting for the next 30-second saved clip buffer.";
  }
  return `
    <section class="panel radar-capture-status ${esc(tone)}">
      <div>
        <span class="eyebrow">Source capture</span>
        <h2>${esc(title)}</h2>
        <p>${esc(detail)}</p>
      </div>
      <div class="radar-capture-actions">
        ${active ? `<button class="primary" data-capture-session="${esc(active.id)}">Capture 30s Now</button>` : ""}
        <button data-nav-jump="builder">Upload Source</button>
        <button data-nav-jump="integrations">Recorder Status</button>
      </div>
    </section>
  `;
}

function radarCandidateScope() {
  const real = realCandidates();
  const visibleCandidates = real;
  const sorted = [...visibleCandidates].sort((a, b) => {
    const updated = new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime();
    if (updated) return updated;
    return Number(b.recordingWindowIndex || 0) - Number(a.recordingWindowIndex || 0);
  });
  return { real, visibleCandidates, sorted };
}

function renderRadarSelectionBar(candidates = [], selectedCandidates = []) {
  const selectedCount = selectedCandidates.length;
  const total = candidates.length;
  if (!total) return "";
  const allVisibleSelected = selectedCount > 0 && selectedCount === total;
  return `
    <div class="radar-selection-bar ${selectedCount ? "active" : ""}">
      <div>
        <span class="eyebrow">${selectedCount ? "Selected clips" : "Clip cleanup"}</span>
        <strong>${selectedCount ? `${selectedCount} selected` : "Select clips to manage them"}</strong>
        <small>${selectedCount ? "Delete bad windows here. Stream monitoring controls live in Stream Watchlist." : "Use the checkboxes, then delete selected clips from this bar."}</small>
      </div>
      <div class="selection-actions">
        <button data-action="select-visible-candidates" ${allVisibleSelected ? "disabled" : ""}>${allVisibleSelected ? "All visible selected" : "Select all visible"}</button>
        <button data-action="clear-selected-candidates" ${selectedCount ? "" : "disabled"}>Clear selection</button>
        <button class="danger strong" data-action="delete-selected-candidates" ${selectedCount ? "" : "disabled"}>Delete ${selectedCount ? `${selectedCount} selected` : "selected"}</button>
      </div>
    </div>
  `;
}
  ...
  return `
    <div class="radar-table-wrap">
      <table class="radar-table">
        <thead>
          <tr>
            <th><input class="clip-select-box" type="checkbox" data-candidate-check-all ${allSelected ? "checked" : ""} aria-label="Select all visible clips"></th>
...
function renderRadarRow(candidate, index) {
  ...
  const checked = state.selectedCandidateIds.includes(candidate.id);
  const streamer = state.streamers.find((item) => item.id === candidate.streamerId);
  const duration = Number(candidate.duration || 0);
  const playback = candidatePlaybackState(candidate);
  const evidence = radarEvidenceLabel(candidate);
  return `
    <tr class="${selected || checked ? "selected" : ""}" data-select-candidate="${candidate.id}">
      <td><input class="clip-select-box" type="checkbox" data-candidate-check="${candidate.id}" ${checked ? "checked" : ""} aria-label="Select ${esc(candidateDisplayTitle(candidate))}"></td>
      <td>
        <div class="radar-candidate-cell">
          ${radarThumb(candidate, index)}
          <div>
            <strong>${esc(candidateDisplayTitle(candidate))}</strong>
            <small>${esc(candidateSummary(candidate)).slice(0, 118)}</small>
          </div>
        </div>
      </td>
      <td>
        <div class="radar-streamer-cell">
          ...
          <div>
            <strong>${esc(streamer?.displayName || "Unknown streamer")} ${isPracticeCandidate(candidate) ? "<em>practice</em>" : "<em>verified</em>"}</strong>
            <small>${streamer?.liveStatus ? esc(liveLabel(streamer.liveStatus)) : "Live monitor"}</small>
          </div>
        </div>
      </td>
      <td><b>${duration || 30}s</b><small class="${durationLabelTone(duration)}">${durationLabel(duration)}</small></td>
      <td>
        <div class="radar-evidence-cell ${esc(evidence.tone)}">
          <b>${esc(evidence.label)}</b>
          <small>${esc(evidence.detail)}</small>
        </div>
      </td>
      <td>
        <div class="radar-row-actions">
          <button data-preview-candidate="${candidate.id}">${playback.playable ? "Play" : "Details"}</button>
          <button class="primary" data-builder-candidate="${candidate.id}">Send to Builder</button>
          <button class="danger" data-delete-candidate="${candidate.id}" title="Delete this clip candidate">Delete</button>
        </div>
      </td>
    </tr>
  `;
  ...
  return `
    <article class="item-card">
      ...
      <div class="actions">
        ...
        <button class="danger" data-delete-candidate="${candidate.id}">Delete</button>
      </div>
    </article>
  `;
}
...
function selectedCandidate() {
  ...
  const real = realCandidates();
  if (selected && !isPracticeCandidate(selected) && candidateIsRadarReady(selected)) return selected;
  return [...real].sort((a, b) => Number(b.score || 0) - Number(a.score || 0))[0] || null;
}
...
function radarThumb(candidate, index = 0) {
  ...
  const playback = candidatePlaybackState(candidate);
  return previewFrame({
    label: isDemo ? "PRACTICE" : "",
    title: candidateDisplayTitle(candidate),
    subtitle: `${streamer?.displayName || "Stream"} · ${sourceTypeLabel(candidate)}`,
    timestamp: start,
    end,
    index,
    imageUrl: candidateThumbnailUrl(candidate),
    candidateId: candidate.id,
    className: `radar-thumb ${playback.playable ? "is-playable" : "is-source-pending"}`,
    score: candidate.score,
    actionLabel: playback.playable ? "Play" : "Details",
...
function candidateTags(candidate, index = 0) {
  ...
  const labels = [
    ...
    playback.playable ? "Playable source" : playback.sourcePending ? "Video pending" : "No source",
    candidate.decision === "accepted" ? "Accepted" : candidate.decision === "rejected" ? "Rejected" : "Review"
  ];
...
function candidateSummary(candidate) {
  const playback = candidatePlaybackState(candidate);
  const streamer = state.streamers.find((item) => item.id === candidate?.streamerId);
  const name = streamer?.displayName || candidate?.streamerName || "This streamer";
  const category = candidate?.category || streamer?.liveCategory || "the live category";
  const title = cleanInline(candidate?.liveTitle || streamer?.liveTitle || candidate?.streamTitle || candidate?.title || "");
  const viewers = Number(candidate?.viewerCount || streamer?.liveViewerCount || 0);
  if (candidate?.transcriptSnippet && playback.playable) return candidate.transcriptSnippet;
  if (candidate?.reason && playback.playable) return candidate.reason;
  const viewerLine = viewers ? `${viewers.toLocaleString()} viewers are on the stream.` : "Viewer data is not available yet.";
  const titleLine = title && !/^30s clip window/i.test(title) ? ` Stream title: ${title}.` : "";
  if (playback.sourcePending) {
    return `${name} is live in ${category}.${titleLine} ${viewerLine} Review the moment, then send it to Builder only if the action or reaction is worth cutting.`;
  }
  return `${name} has a candidate window in ${category}.${titleLine} ${playback.playable ? "Playable source is attached for review." : "Attach source before packaging."}`;
}
...
function radarEvidenceLabel(candidate) {
  const playback = candidatePlaybackState(candidate);
  if (playback.playable) return { label: "Playable", detail: "Ready to review", tone: "good" };
  if (playback.sourcePending) return { label: "Needs video", detail: "Open live or upload source", tone: "warn" };
  return { label: "Needs source", detail: "Attach media before build", tone: "warn" };
}

function sourceTypeLabel(candidate) {
  if (candidate?.sourceType === "live_recording_window") return "Live metadata window";
  if (candidate?.sourceType === "upload") return "Uploaded media";
  if (candidate?.sourceType === "practice" || candidate?.sourceType === "demo_media") return "Practice media";
  return candidate?.sourceType || "Unknown source";
}
...
function normalizedSignalSource(value) {
  return String(value || "").toLowerCase();
}

function hasRealChatSignal(candidate) {
  const source = normalizedSignalSource(candidate?.chatSignals?.source);
  const spike = Number(candidate?.chatSignals?.spike || candidate?.chatSignals?.messagesPerMinute || 0);
  return Boolean(spike && source && ![
    "watch_window_estimate",
    "agent101_demo",
    "practice_signal",
    "unavailable",
...
function scoreEvidence(candidate) {
  const provider = normalizedSignalSource(candidate?.scoringProvider || candidate?.scoreEvidence?.source);
  const isRealSource = ["openai", "operator", "verified_media_analysis", "provider_chat", "transcript_chat"].includes(provider);
  const isPractice = isPracticeCandidate(candidate);
  const hasScore = Number.isFinite(Number(candidate?.score)) && Number(candidate?.score) > 0;
  if (isRealSource && hasScore) {
    return {
      real: true,
      value: Math.round(Number(candidate.score)),
      label: provider === "openai" ? "AI review score" : provider === "operator" ? "Operator score" : "Review score",
      detail: "Evidence-backed"
    };
  }
  if (isPractice && hasScore) {
    return {
      real: false,
      value: Math.round(Number(candidate.score)),
  ...
  return {
    ...
    value: null,
    label: "Review score",
    detail: "Needs real analysis"
  };
}

function hookEvidence(candidate) {
  const evidence = scoreEvidence(candidate);
  const hook = Number(candidate?.hookScore);
  if (evidence.real && Number.isFinite(hook) && hook > 0) {
    return { value: Math.round(hook), label: "Hook strength", detail: evidence.detail };
  }
  if (isPracticeCandidate(candidate) && Number.isFinite(hook) && hook > 0) {
    return { value: Math.round(hook), label: "Practice hook", detail: "Demo only" };
  }
  return { value: null, label: "Hook strength", detail: "Needs transcript or visual analysis" };
}

function chatEvidence(candidate) {
  if (hasRealChatSignal(candidate)) {
    return {
      value: formatEngagement(candidate, 3),
      label: engagementLabel(candidate),
      detail: "Provider chat data"
    };
  }
  if (candidate?.viewerCount) {
    return {
      value: formatEngagement({ ...candidate, sourceType: "live_recording_window" }, 3),
      label: "Live viewers",
      detail: "Viewer count only"
    };
  }
  return {
    value: null,
...
function metricCard(metric) {
  const value = metric.value === null || metric.value === undefined || metric.value === "" ? "Needs analysis" : metric.value;
  return `<span><b>${esc(value)}</b><em>${esc(metric.label)} · ${esc(metric.detail)}</em></span>`;
}

function formatEngagement(candidate, index = 0) {
  if (candidate?.sourceType === "live_recording_window") {
    const viewers = Number(candidate.viewerCount || 0);
    if (viewers) return viewers >= 1000 ? `${(viewers / 1000).toFixed(viewers >= 10000 ? 1 : 1)}K` : String(viewers);
    return "Pending";
  }
  if (candidate?.chatSignals?.source === "UNAVAILABLE" || (candidate?.viewerCount == null && candidate?.sourceProvenance === "DEMO_SOURCE")) {
    return "Unavailable";
  }
  if (!hasRealChatSignal(candidate)) return "Unavailable";
  const spike = Number(candidate.chatSignals?.spike || candidate.chatSignals?.messagesPerMinute || 0);
  const value = spike * 420;
  return value >= 1000 ? `${(value / 1000).toFixed(value >= 10000 ? 1 : 1)}K` : String(Math.round(value));
}

function engagementLabel(candidate) {
  if (candidate?.sourceType === "live_recording_window") return hasRealChatSignal(candidate) ? "Chat spike" : candidate.viewerCount ? "Live viewers" : "Chat pending";
  return "Chat spike";
}
...
function candidateStatusBadge(status, candidate = null) {
  if (candidate && candidateNeedsWatcherCleanup(candidate)) return badge("Needs video", "warn");
  if (status === "packaged") return badge("Packaged", "good");
  if (status === "rejected") return badge("Deleted", "bad");
  if (status === "source_pending") return badge("Needs video", "warn");
  if (status === "recording") return badge("Recording", "info");
  if (status === "reviewed") return badge("Review", "info");
  return badge("New", "warn");
}

function renderCandidateInspector(candidate) {
  const streamer = state.streamers.find((item) => item.id === candidate.streamerId);
  const score = scoreEvidence(candidate);
  const hook = hookEvidence(candidate);
  const engagement = chatEvidence(candidate);
  const playback = candidatePlaybackState(candidate);
  const watchSession = (state.activeWatchSessions || []).find((session) => session.id === candidate.watchSessionId);
  return `
    <div class="inspector-head review-command-head">
      <div>
        <h2>Selected Clip</h2>
        <p>Review the window, then move it forward.</p>
      </div>
      ${playback.playable
        ? `<button class="primary review-next-button" data-builder-candidate="${esc(candidate.id)}">Send to Builder</button>`
        : watchSession?.id
          ? `<button class="primary review-next-button" data-capture-session="${esc(watchSession.id)}">Capture 30s</button>`
          : `<button class="primary review-next-button" data-nav-jump="builder">Upload Source</button>`}
    </div>
    ${radarThumb(candidate, 1)}
    <div class="selected-title review-selected-title">
      <div>
        <h3>${esc(candidateDisplayTitle(candidate))}</h3>
        <p>${esc(streamer?.displayName || "Unknown streamer")} · ${esc(sourceTypeLabel(candidate))} · ${timeAgo(candidate.updatedAt || candidate.createdAt)}</p>
      </div>
      ${score.real ? scoreRing(score.value) : ""}
    </div>
    <div class="inspector-section review-summary">
      <h3>${playback.playable ? "Why this clip is here" : "What Agent 101 captured"}</h3>
      <p>${esc(candidateSummary(candidate))}</p>
      <div class="clip-metrics">
        ${metricCard(score)}
        ${metricCard(hook)}
        ${metricCard(engagement)}
        <span><b>${playback.playable ? "Attached" : "Needs video"}</b><em>Source</em></span>
      </div>
    </div>
    <div class="inspector-section compact-actions review-secondary-actions">
      <h3>Actions</h3>
      <button data-preview-candidate="${candidate.id}">${playback.playable ? "Play Clip" : "Open Details"}</button>
      <button class="danger" data-delete-candidate="${candidate.id}">Delete</button>
    </div>
  `;
}

function renderBulkCandidateInspector(candidates = [], visibleCount = 0) {
  const streamers = [...new Set(candidates.map((candidate) => streamerName(candidate.streamerId)))].slice(0, 3);
  const needsVideo = countWhere(candidates, (candidate) => candidatePlaybackState(candidate).sourcePending);
  const scoreAverage = candidates.length
    ? Math.round(candidates.reduce((total, candidate) => total + Number(candidate.score || 0), 0) / candidates.length)
    : 0;
  return `
    <div class="bulk-inspector">
      <div class="inspector-head">
        <div>
          <h2>${esc(candidates.length)} Clips Selected</h2>
          <p>${esc(streamers.join(", ") || "Multiple streamers")} · ${esc(visibleCount)} visible</p>
        </div>
        ${scoreRing(scoreAverage)}
      </div>
      <div class="inspector-section">
        <h3>Bulk Cleanup</h3>
        <p>Use this for clearing bad 30s windows fast. This only removes local Radar candidates and does not touch Twitch, external accounts, or source media.</p>
        <div class="clip-metrics">
          <span><b>${candidates.length}</b><em>Selected</em></span>
          <span><b>${needsVideo}</b><em>Need video</em></span>
          <span><b>${scoreAverage}</b><em>Average score</em></span>
        </div>
      </div>
      <div class="inspector-section compact-actions bulk-actions">
        <h3>Selection Actions</h3>
        <button class="danger" data-action="delete-selected-candidates">Delete ${esc(candidates.length)} Selected</button>
        <button data-action="clear-selected-candidates">Clear Selection</button>
        <button data-action="select-visible-candidates">Select All Visible</button>
      ...
      <div class="inspector-section">
        ...
        <div class="bulk-selected-list">
          ${candidates.slice(0, 8).map((candidate) => `
            <button data-select-candidate="${esc(candidate.id)}">
              <b>${esc(candidateDisplayTitle(candidate))}</b>
              <span>${esc(timeAgo(candidate.updatedAt || candidate.createdAt))} · ${esc(candidate.status || "candidate")}</span>
            </button>
          `).join("")}
        </div>
      </div>
...
function renderClipPreviewModal() {
  ...
  const playbackUrl = source?.playbackUrl || candidate.playbackUrl;
  const sourceUrl = streamerSourceUrl(streamer);
  const watchSession = (state.activeWatchSessions || []).find((session) => session.id === candidate.watchSessionId);
  const score = scoreEvidence(candidate);
  const hook = hookEvidence(candidate);
  const engagement = chatEvidence(candidate);
  return `
    <div class="clip-preview-overlay" role="dialog" aria-modal="true" aria-label="Clip preview">
      <section class="clip-preview-modal ${playback.playable ? "is-playback" : "is-live-source"}">
        <div class="preview-head">
          <div>
            <span class="eyebrow">${playback.playable ? "Clip playback" : "Source not saved"}</span>
            <h2>${esc(candidateDisplayTitle(candidate))}</h2>
            <p>${esc(streamer?.displayName || "Unknown streamer")} · ${esc(sourceTypeLabel(candidate))} · ${esc(candidate.timestampStart || "00:00")} to ${esc(candidate.timestampEnd || `${candidate.duration || 30}s`)}</p>
          </div>
          ...
          <div class="preview-player">
            ${playbackUrl ? `
              <div class="clip-preview-video-wrap">
                ...
                <video class="studio-player clip-preview-video" src="${esc(appUrl(playbackUrl))}" controls playsinline preload="metadata" data-studio-video data-start="${esc(candidateStartSeconds(candidate))}" data-window-duration="${esc(candidateWindowDuration(candidate))}"></video>
              </div>
            ` : renderSourcePendingPreview(candidate, streamer, playback)}
            ${playback.playable ? "" : renderPreviewTransport(candidate, streamer, playback)}
          </div>
          <aside class="preview-detail">
            <div class="preview-score-row">
              ${score.real ? scoreRing(score.value) : metricCard(score)}
              ${metricCard(hook)}
              ${metricCard(engagement)}
            </div>
            <section>
            ...
            <section>
              ...
              <p class="transcript-box">${esc(candidate.transcriptSnippet || (playback.playable ? "No transcript captured yet. Add notes or run scoring after review." : "No transcript has been extracted from this live-source window yet."))}</p>
            </section>
            <div class="preview-actions">
              ${playback.playable
                ? `<button class="primary" data-preview-open-builder="${esc(candidate.id)}">Send to Builder</button>`
                : watchSession?.id
                  ? `<button class="primary" data-capture-session="${esc(watchSession.id)}">Capture 30s Now</button>`
                  : `<button class="primary" data-nav-jump="builder">Upload Source</button>`}
              ${!playback.playable && sourceUrl ? `<button data-open-streamer-source="${esc(streamer?.id || "")}">Open Live Source</button>` : ""}
              <button class="danger" data-delete-candidate="${esc(candidate.id)}">Delete</button>
...
function renderLiveMonitorModal() {
  const streamer = state.streamers.find((item) => item.id === state.livePreviewStreamerId);
  if (!streamer) return "";
  const session = activeWatchSessionForStreamer(streamer.id);
  const readiness = clipQualityForStreamer(streamer);
  const savedCandidates = savedRadarCandidatesForStreamer(streamer.id);
  const pendingWindows = pendingWatchWindowsForStreamer(streamer.id);
  const sourceUrl = streamerSourceUrl(streamer);
  const thumbnail = streamThumbnailUrl(streamer);
  const live = isStreamerLive(streamer);
  const captureBlocked = session?.captureStatus === "blocked" || readiness.captureBlocked;
  const savedLabel = savedCandidates.length
    ? `${savedCandidates.length} saved Radar clip${savedCandidates.length === 1 ? "" : "s"}`
    : pendingWindows.length
      ? `${pendingWindows.length} window${pendingWindows.length === 1 ? "" : "s"} waiting for save`
      : "No saved Radar clip yet";
  const statusLine = session?.captureStatus === "capturing"
    ? session.captureMessage || "Saving local clip buffer."
    : captureBlocked
      ? session?.captureMessage || state.media?.recorder?.message || "Recorder is blocked. Connect streamlink or yt-dlp before Radar can store playback."
      : savedCandidates.length
        ? "Saved playback is ready in Clip Radar."
        : "This is a live monitor. Save a 30s buffer before it becomes Clip Radar playback.";
  return `
    <div class="clip-preview-overlay" role="dialog" aria-modal="true" aria-label="Live stream monitor">
      <section class="clip-preview-modal live-monitor-modal">
        <div class="preview-head">
          <div>
            <span class="eyebrow">Live monitor</span>
            <h2>${esc(streamer.displayName || "Live stream")}</h2>
            <p>${esc(live ? "Live now" : liveStatusMeta(streamer).label)} · ${esc(streamer.platform || "stream")} · ${streamer.liveViewerCount ? `${Number(streamer.liveViewerCount).toLocaleString()} viewers` : "viewer count pending"}</p>
          </div>
          <button class="icon-button" data-close-live-preview aria-label="Close live monitor">×</button>
        </div>
        <div class="preview-body live-monitor-body">
          <div class="preview-player live-monitor-player">
            <div class="source-pending-preview live-monitor-frame">
              ${thumbnail ? `<img class="source-live-image" src="${esc(thumbnail)}" alt="" loading="lazy">` : ""}
              <div class="live-monitor-overlay">
                <span class="source-status-pill">${esc(live ? "LIVE NOW" : liveStatusMeta(streamer).label)}</span>
                <h3>${esc(streamer.liveTitle || `${streamer.displayName} live stream`)}</h3>
                <p>${esc(streamer.liveCategory || "Live source")}</p>
              </div>
            </div>
            <div class="preview-transport live-monitor-transport">
              ${session?.id ? `<button class="primary" type="button" data-capture-session="${esc(session.id)}">${savedCandidates.length ? "Save another 30s to Radar" : "Save 30s to Radar"}</button>` : `<button type="button" data-toggle-monitor="${esc(streamer.id)}">Start watcher</button>`}
              ${sourceUrl ? `<button type="button" data-open-streamer-source="${esc(streamer.id)}">Open live source</button>` : ""}
              <div class="window-clock">
                <span><b>${esc(session?.currentStage || (live ? "Live monitor active" : "Not live"))}</b><em>${esc(savedLabel)}</em></span>
                <strong>${esc(statusLine)}</strong>
              </div>
            </div>
          </div>
          <aside class="preview-detail live-monitor-detail">
            ${renderClipReadinessMeter(streamer)}
            <section>
              <h3>Radar status</h3>
              <p>${esc(statusLine)}</p>
              <div class="clip-metrics">
                <span><b>${esc(savedCandidates.length)}</b><em>Saved clips</em></span>
                <span><b>${esc(pendingWindows.length)}</b><em>Pending windows</em></span>
                <span><b>${esc(session?.captureStatus || "idle")}</b><em>Recorder</em></span>
              </div>
            </section>
            <section>
              <h3>Watch state</h3>
              <p>${esc(session?.id ? `Session ${session.id} is ${session.status || "active"}. Last media: ${fmtDate(session.lastMediaAt)}.` : "No active backend watcher exists for this stream.")}</p>
            </section>
            <div class="preview-actions">
              <button class="primary" data-nav-jump="radar">Open Clip Radar</button>
              <button data-nav-jump="watchlist">Open Watchlist</button>
              <button data-close-live-preview>Close</button>
            </div>
          </aside>
        </div>
      </section>
...
function renderPreviewTransport(candidate, streamer, playback) {
  const sourceUrl = streamerSourceUrl(streamer);
  const duration = candidateWindowDuration(candidate);
  const range = candidateWindowRangeLabel(candidate);
  return `
    <div class="preview-transport window-clock-transport">
      ${candidate.watchSessionId ? `<button type="button" data-capture-session="${esc(candidate.watchSessionId)}">Capture 30s</button>` : `<button type="button" data-nav-jump="builder">Upload source</button>`}
      ${sourceUrl ? `<button type="button" data-open-streamer-source="${esc(streamer?.id || "")}">Open live</button>` : ""}
      <div class="window-clock">
        <span><b>${esc(range)}</b><em>${esc(duration)}s target window</em></span>
        <strong>No saved clip file yet</strong>
      </div>
    </div>
  `;
}

function renderSourcePendingPreview(candidate, streamer, playback) {
  const thumbnail = candidateThumbnailUrl(candidate);
  const session = (state.activeWatchSessions || []).find((item) => item.id === candidate.watchSessionId);
  return `
    <div class="source-pending-preview clean">
      ${thumbnail ? `<img class="source-live-image muted" src="${esc(thumbnail)}" alt="" loading="lazy">` : ""}
      <div class="source-empty-frame">
        <strong>No saved clip file yet</strong>
        <span>${esc(session?.captureMessage || playback.detail || "Capture or upload source media before this can become a Builder clip.")}</span>
      </div>
      <span class="source-status-pill">${esc(playback.label)}</span>
    </div>
  `;
}
...
function renderBuilder() {
  ...
  const candidates = builderCandidateList(studio);
  ...
  const renderedArtifact = activeJob?.artifactId
    ...
    : candidate?.renderedArtifactId
      ...
      : null;
  const activeSource = builderActiveSource(source, candidate);
  builderEditState(project);
  if (!candidate) {
    view.innerHTML = `
      <section class="builder-page capcut-builder-page">
        <header class="panel capcut-builder-hero">
          <div>
            <span class="eyebrow">Clip Builder</span>
            <h2>CapCut Agent workspace</h2>
            <p>Select a saved clip from Clip Radar. Agent 101 will stage the edit, request Human Gate approval when needed, and run the CapCut workflow from here.</p>
          </div>
          <div class="capcut-builder-actions">
            <button class="primary" type="button" data-nav-jump="radar">Choose from Clip Radar</button>
            <button type="button" data-action="refresh">Refresh</button>
          </div>
        </header>
        <div class="capcut-builder-empty-grid">
          <main class="panel capcut-builder-empty-card">
            <span class="eyebrow">Ready for source</span>
            <h3>No clip selected</h3>
            <p>Clip Builder only opens real saved candidates now. Pick a clip in Radar, then run the CapCut Agent from the Builder panel.</p>
            <button class="primary" type="button" data-nav-jump="radar">Open Clip Radar</button>
          </main>
          <aside class="panel capcut-builder-status-card">
            <span class="eyebrow">Agent pipeline</span>
            <h3>${esc(candidates.length)} staged clip${candidates.length === 1 ? "" : "s"}</h3>
            <p>${esc(candidates.length ? "Select one from Radar to start the edit." : "No Builder-ready clip is staged yet.")}</p>
            <div class="capcut-builder-steps">
              <span><b>1</b>Choose clip</span>
              <span><b>2</b>Render MP4</span>
              <span><b>3</b>Start CapCut Agent</span>
              <span><b>4</b>Human Gate export</span>
            </div>
          </aside>
        </div>
      </section>
    ...
    <section class="builder-page capcut-builder-page">
      <header class="panel capcut-builder-hero">
        <div>
          <span class="eyebrow">Clip Builder</span>
          <h2>${esc(candidateDisplayTitle(candidate))}</h2>
          <p>Review the saved clip, render a verified MP4 if needed, then let Agent 101 run the CapCut edit workflow from this panel.</p>
        </div>
        <div class="capcut-builder-actions">
          <button type="button" data-nav-jump="radar">Back to Radar</button>
          <button class="primary" type="button" data-builder-command="export" ${disabledAttr(!(readiness.canRender && !state.studioBusy), readiness.renderReasons?.[0] || "Render setup is not ready")}>${state.studioBusy ? "Rendering..." : "Render MP4"}</button>
        </div>
      </header>
      <div class="capcut-builder-layout">
        <main class="panel capcut-builder-preview">
          ${renderBuilderCanvas(activeSource, candidate)}
        </main>
        <aside class="panel capcut-builder-agent-panel">
          ${renderBuilderCapcutAgent(candidate, renderedArtifact, readiness)}
          <div class="builder-action-stack">
            <button type="button" data-builder-command="create-package" ${readiness.canPackage ? "" : "disabled"}>${state.packages.some((item) => item.candidateId === candidate.id) ? "Package ready" : "Create package"}</button>
            <button type="button" data-nav-jump="radar">Choose another clip</button>
          </div>
        </aside>
      </div>
    </section>
...
function builderActiveSource(source, candidate) {
  const playback = candidatePlaybackState(candidate);
  const safeSource = isPracticeSource(source) ? null : source;
  if (playback.source?.playable) return playback.source;
  if (safeSource?.playable) return safeSource;
  return playback.source || safeSource || null;
}

function builderCandidateList(studio = {}) {
  const selected = state.candidates.find((candidate) => candidate.id === state.selectedCandidateId);
  const queued = builderQueueCandidates();
  const studioCandidates = Array.isArray(studio.candidates) ? studio.candidates : [];
  const base = [
    ...queued,
    ...studioCandidates.filter((candidate) =>
      !isPracticeCandidate(candidate) &&
      candidateInBuilder(candidate)
    )
  ];
  if (selected && !isPracticeCandidate(selected) && candidateInBuilder(selected) && !base.some((candidate) => candidate.id === selected.id)) base.unshift(selected);
  return base
    .filter((candidate, index, list) => list.findIndex((item) => item.id === candidate.id) === index)
    .sort((a, b) => new Date(b.builderDraft?.updatedAt || b.updatedAt || b.createdAt || 0) - new Date(a.builderDraft?.updatedAt || a.updatedAt || a.createdAt || 0))
    .slice(0, 80);
}

function renderBuilderTopbar(project, candidate, readiness) {
  const edit = builderEditState(project);
  const title = candidate ? candidateDisplayTitle(candidate) : project.title || "No clip selected";
  return `
    <header class="panel builder-topbar">
      <div class="builder-topbar-title">
        <span class="builder-app-mark">A</span>
        <div>
          <strong>Clip Builder</strong>
          <small>${esc(title)}</small>
        </div>
      </div>
      <div class="builder-aspect-switch" aria-label="Aspect ratio">
        ${["9:16", "1:1", "16:9", "4:5", "21:9"].map((ratio) => `
          <button class="${edit.aspectRatio === ratio ? "active" : ""}" data-builder-aspect="${esc(ratio)}">${esc(ratio)}</button>
        `).join("")}
      </div>
      <div class="builder-topbar-actions">
        <button class="icon-button" type="button" data-builder-command="undo" title="Undo">Undo</button>
        <button class="icon-button" type="button" data-builder-command="redo" title="Redo">Redo</button>
        <button class="primary" type="button" data-builder-command="ai-auto-edit">AI Auto Edit</button>
        <button type="button" data-builder-command="save">Save</button>
        <button class="primary export" type="button" data-builder-command="export" ${disabledAttr(!(readiness.canRender && !state.studioBusy), readiness.renderReasons?.[0] || "Render setup is not ready")}>${state.studioBusy ? "Exporting..." : "Export"}</button>
      </div>
    </header>
  `;
}

function builderToolItems() {
  return [
    ["media", "Media", "M"],
    ["assets", "Assets", "A"],
...
function renderBuilderToolRail() {
  return `
    <nav class="builder-tool-rail" aria-label="Clip Builder tools">
      ${builderToolItems().map(([id, label, icon]) => `
        <button class="${state.builderTool === id ? "active" : ""}" type="button" data-builder-tool="${esc(id)}" title="${esc(label)}">
          <span>${esc(icon)}</span>
          <small>${esc(label)}</small>
        </button>
      `).join("")}
...
function builderToolCard(label, command, detail = "") {
  return `
    <button class="builder-tool-card" type="button" data-builder-command="${esc(command)}">
...
function builderToggle(label, path, detail = "") {
  const checked = Boolean(builderValueAt(path));
  return `
    <label class="builder-toggle">
      <span><b>${esc(label)}</b>${detail ? `<small>${esc(detail)}</small>` : ""}</span>
      <input type="checkbox" data-builder-setting="${esc(path)}" ${checked ? "checked" : ""}>
    </label>
  `;
}

function builderSlider(label, path, min, max, suffix = "%") {
  const value = Number(builderValueAt(path) ?? 0);
  return `
    <label class="builder-setting">
      <span><b>${esc(label)}</b><em>${esc(value)}${esc(suffix)}</em></span>
      <input type="range" min="${esc(min)}" max="${esc(max)}" value="${esc(value)}" data-builder-setting="${esc(path)}">
    </label>
  `;
}

function builderSelect(label, path, options = []) {
  const value = builderValueAt(path);
  return `
    <label class="builder-select">
      <span>${esc(label)}</span>
      <select data-builder-setting="${esc(path)}">
...
function renderBuilderToolDrawer(source, candidate, readiness) {
  const tool = state.builderTool || "ai";
  const title = builderToolItems().find(([id]) => id === tool)?.[1] || "AI Tools";
  const packageReady = candidate ? Boolean(selectedClipPackage(candidate)) : false;
  const mediaPanel = `
    <div class="builder-tool-section">
      <h4>Source</h4>
      <p>${esc(source?.playable ? "Verified playable media is attached." : "Attach source before final export.")}</p>
      <form id="media-upload-form" class="builder-upload-mini" enctype="multipart/form-data">
        <input name="title" value="${esc(candidate ? candidateDisplayTitle(candidate) : "Clip source")}" type="hidden">
        <input name="file" type="file" accept="video/mp4,video/webm,video/quicktime,.mp4,.webm,.mov" required>
        <button class="primary" type="submit">Upload source</button>
      </form>
      ${candidate?.watchSessionId ? `<button type="button" data-capture-session="${esc(candidate.watchSessionId)}">Capture 30s now</button>` : ""}
      ${builderToolCard("Capture frame", "capture-frame", "Open exact source frame")}
    </div>
  `;
  const panels = {
    media: mediaPanel,
    assets: `
      <div class="builder-tool-section">
        <h4>Assets</h4>
        ${builderToolCard("Add logo layer", "add-sticker", "Creates a movable overlay")}
        ${builderToolCard("Duplicate layer", "duplicate-layer", "Clone selected overlay/effect")}
        ${builderToolCard("Delete layer", "delete-layer", "Remove selected layer")}
      </div>
    `,
    templates: `
      <div class="builder-tool-section preset-grid">
        <button data-builder-preset="tiktok-gaming">TikTok Gaming<small>Reframe, zoom, captions</small></button>
        <button data-builder-preset="podcast-reaction">Podcast Reaction<small>Face track, clean captions</small></button>
        <button data-builder-preset="clean-export">Clean Export<small>No captions or zoom</small></button>
      </div>
    `,
    text: `
      <div class="builder-tool-section">
        <h4>Text</h4>
        ${builderToolCard("Add title", "add-text", "Adds a text layer to timeline")}
        ${builderSelect("Caption style", "captions.style", ["Gaming Bold", "Clean Creator", "Neon Pop", "Minimal", "None"])}
      </div>
    `,
    captions: `
      <div class="builder-tool-section">
        <h4>Captions</h4>
        ${builderToggle("Auto captions", "captions.enabled", packageReady ? "Caption files can be generated" : "Package first to write files")}
        ${builderToggle("Word-by-word", "captions.wordByWord", "TikTok style timing")}
        ${builderToggle("Emphasis words", "captions.emphasis", "Highlight punch words")}
        ${builderToolCard("Generate captions", "captions", "Uses package caption writer when ready")}
      </div>
    `,
    audio: `
      <div class="builder-tool-section">
        <h4>Sound Studio</h4>
        ${builderToggle("Voice enhance", "sound.enhance", "Local edit state")}
        ${builderToggle("Normalize loudness", "sound.normalize", "Export target setting")}
        ${builderToggle("Duck music", "sound.duckMusic", "Keep voice clear")}
      </div>
    `,
    stickers: `
      <div class="builder-tool-section">
        <h4>Stickers</h4>
        ${builderToolCard("Add reaction sticker", "add-sticker", "Adds a sticker lane item")}
        ${builderToolCard("Track object", "add-motion", "Adds tracking keyframe")}
      </div>
    `,
    effects: `
      <div class="builder-tool-section">
        <h4>Effects</h4>
        ${builderToolCard("Add impact effect", "add-effect", "Adds effect lane block")}
        ${builderSlider("Auto zoom intensity", "autoZoom.intensity", 0, 100)}
        ${builderToggle("Auto zoom enabled", "autoZoom.enabled", "Reaction and action triggers")}
      </div>
    `,
    transitions: `
      <div class="builder-tool-section">
        <h4>Transitions</h4>
        ${builderToolCard("Add flash transition", "add-effect", "Creates a transition block")}
        ${builderToolCard("Split at playhead", "split", "Adds a split marker")}
      </div>
    `,
    filters: `
      <div class="builder-tool-section">
        <h4>Filters</h4>
        ${builderSelect("Background", "background.mode", ["blur", "heavy blur", "soft gradient", "mirror", "dark", "glow"])}
        ${builderSlider("Background strength", "background.strength", 0, 100)}
      </div>
    `,
    ai: `
      <div class="builder-tool-section ai-tools-grid">
        ${builderToolCard("Auto Reframe", "preset:tiktok-gaming", "Follow subject into 9:16")}
        ${builderToolCard("Side Blur", "preset:podcast-reaction", "Clean vertical fill")}
        ${builderToolCard("Auto Zoom", "add-motion", "Add punch zoom keyframes")}
        ${builderToolCard("AI Track", "add-motion", "Track subject layer")}
        ${builderToolCard("Auto Captions", "captions", "Create caption layer/files")}
        ${builderToolCard("AI Edit", "ai-auto-edit", "Apply full editor preset")}
      </div>
      <div class="builder-tool-section">
        ${builderSelect("Tracking target", "reframe.target", ["Smart AI", "Face", "Crosshair / player", "Mouse", "Largest Motion"])}
        ${builderSlider("Sensitivity", "reframe.sensitivity", 0, 100)}
        ${builderSlider("Smoothness", "reframe.smoothness", 0, 100)}
      </div>
    `
  };
  return `
    <aside class="panel builder-tool-drawer">
      <div class="builder-drawer-head">
        <span class="eyebrow">${esc(title)}</span>
        <strong>${esc(candidate ? candidateDisplayTitle(candidate) : "Select a clip")}</strong>
      </div>
      ${panels[tool] || panels.ai}
      <div class="builder-readiness-card">
        <b>${readiness.canRender ? "Render ready" : "Needs source/range"}</b>
        <small>${esc(readiness.canRender ? "Export uses the verified render route." : readiness.renderReasons?.[0] || "Select a playable clip.")}</small>
      </div>
    </aside>
  `;
}

function renderBuilderCapcutAgent(candidate, renderedArtifact, readiness) {
  const verifiedArtifact = renderedArtifactForCandidate(candidate, renderedArtifact);
  const activeSession = activeCapcutSessionForCandidate(candidate);
  const edit = builderEditState();
  const busy = Boolean(state.capcutAgentBusy);
  const canStart = Boolean(verifiedArtifact && !busy);
  const practice = isPracticeCandidate(candidate);
  const statusLabel = verifiedArtifact
    ? state.capcut?.agentReady ? "Ready" : "Needs CapCut login"
    : "Render needed";
  return `
    <section class="builder-capcut-agent">
      <div class="builder-capcut-head">
        <div>
          <span class="eyebrow">CapCut Agent</span>
          <h3>Agent edit run</h3>
        </div>
        ${badge(statusLabel, verifiedArtifact ? state.capcut?.agentReady ? "good" : "warn" : "warn")}
      </div>
      ${verifiedArtifact ? `
        <form id="builder-capcut-agent-form" class="builder-capcut-form">
          <input type="hidden" name="clip_id" value="${esc(candidate.id)}">
          <input type="hidden" name="rendered_artifact_id" value="${esc(verifiedArtifact.id)}">
          <div class="builder-capcut-grid">
            <label><span>Format</span><select name="aspect_ratio"><option value="9:16" ${edit.aspectRatio === "9:16" ? "selected" : ""}>9:16</option><option value="1:1" ${edit.aspectRatio === "1:1" ? "selected" : ""}>1:1</option><option value="16:9" ${edit.aspectRatio === "16:9" ? "selected" : ""}>16:9</option></select></label>
            <label><span>Platform</span><select name="platform"><option value="tiktok">TikTok</option><option value="instagram_reels">Instagram</option><option value="youtube_shorts">Shorts</option></select></label>
          </div>
          <div class="builder-capcut-checks">
            <label class="inline-check"><input type="checkbox" name="auto_captions" checked> Captions</label>
            <label class="inline-check"><input type="checkbox" name="auto_reframe" checked> Reframe</label>
          </div>
          <textarea name="hook_text" rows="2" placeholder="Edit note or hook for Agent 101">${esc(candidate?.suggestedHook || "")}</textarea>
          ${practice ? `<label class="inline-check practice-confirm"><input type="checkbox" name="practice_confirmed"> Confirm practice media</label>` : ""}
          <button class="primary" type="submit" ${canStart ? "" : disabledAttr(true, busy ? "CapCut Agent is already running" : "Render a verified MP4 first")}>${busy ? "Agent running..." : "Start CapCut Agent"}</button>
        </form>
      ` : `
        <div class="builder-capcut-blocked">
          <b>Render the clip before CapCut Agent starts.</b>
          <p>The agent edits a saved MP4, not a live preview or metadata-only row.</p>
          <button class="primary" type="button" data-builder-command="export" ${disabledAttr(!(readiness.canRender && !state.studioBusy), readiness.renderReasons?.[0] || "Render setup is not ready")}>Render draft</button>
        </div>
      `}
      <div class="builder-capcut-run">
        <h4>${esc(activeSession?.title || "Agent progress")}</h4>
        ${renderCapcutAgentEventList(activeSession, 4)}
      </div>
    </section>
  `;
}
...
function renderBuilderPendingSummary(candidate) {
  const streamer = state.streamers.find((item) => item.id === candidate.streamerId);
  const playback = candidatePlaybackState(candidate);
  const queueCount = builderQueueCandidates().length;
  const duration = candidateWindowDuration(candidate);
  return `
    <section class="panel studio-project-summary pending-source-summary builder-queue-summary">
      <div>
        <span class="eyebrow">Builder queue</span>
        <strong>${esc(queueCount)} clip${queueCount === 1 ? "" : "s"} staged</strong>
        <small>${esc(streamer?.displayName || "Unknown streamer")} selected</small>
      </div>
      <div>
        <span class="eyebrow">Current step</span>
        <strong>Review source</strong>
        <small>${esc(candidateWindowRangeLabel(candidate))} · ${esc(duration)}s window</small>
      </div>
      <div>
        <span class="eyebrow">Source status</span>
        <strong>${esc(playback.playable ? "Playable" : "Needs video")}</strong>
        <small>${esc(playback.playable ? "Verified playback attached" : "Upload or open matching source")}</small>
      </div>
      <div>
        <span class="eyebrow">Next move</span>
        <strong>${esc(playback.playable ? "Build draft" : "Attach source")}</strong>
        <small>${esc(playback.playable ? "Ready for render checks." : "Upload or open the live source.")}</small>
      </div>
    </section>
  `;
}
...
function candidateWindowDuration(candidate) {
  const explicit = Number(candidate?.durationSeconds ?? candidate?.duration ?? 0);
  if (explicit > 0) return explicit;
  const derived = candidateEndSeconds(candidate) - candidateStartSeconds(candidate);
  return derived > 0 ? derived : 30;
}

function candidateWindowRangeLabel(candidate) {
  const start = candidateStartSeconds(candidate);
  const end = candidateEndSeconds(candidate) || start + candidateWindowDuration(candidate);
  return `${formatClock(start)} - ${formatClock(end)}`;
}

function builderQueueCandidates() {
  const selected = state.candidates.find((candidate) => candidate.id === state.selectedCandidateId);
  const queue = (state.candidates || []).filter((candidate) =>
    !isPracticeCandidate(candidate) &&
    candidateInBuilder(candidate)
  );
  if (selected && !isPracticeCandidate(selected) && candidateInBuilder(selected) && !queue.some((candidate) => candidate.id === selected.id)) queue.unshift(selected);
  return queue
    .filter((candidate, index, list) => list.findIndex((item) => item.id === candidate.id) === index)
    .sort((a, b) => new Date(b.builderDraft?.updatedAt || b.updatedAt || b.createdAt || 0) - new Date(a.builderDraft?.updatedAt || a.updatedAt || a.createdAt || 0));
}
...
function renderBuilderCanvas(source, candidate) {
  if (!candidate) {
    return `
      <div class="builder-canvas builder-preview-shell empty">
        <div>
          <span class="eyebrow">Editor</span>
          <h3>No clip selected</h3>
          <div class="empty-actions">
            <button class="primary" data-nav-jump="radar">Choose from Clip Radar</button>
          </div>
        </div>
      </div>
    `;
  ...
  const poster = studioCandidateThumb(candidate, 0);
  const start = candidateStartSeconds(candidate);
  const duration = candidateWindowDuration(candidate);
  const playable = Boolean(source?.playable && src);
  const edit = builderEditState();
  const scale = Number(edit.transform?.scale || 100) / 100;
  const translateX = Number(edit.transform?.x || 0);
  const translateY = Number(edit.transform?.y || 0);
  const rotate = Number(edit.transform?.rotate || 0);
  const safeClass = edit.aspectRatio === "16:9" ? "wide" : edit.aspectRatio === "1:1" ? "square" : edit.aspectRatio === "4:5" ? "four-five" : "vertical";
  return `
    <section class="builder-canvas builder-preview-shell ${playable ? "ready" : "missing-source"}">
      <div class="builder-preview-head">
        <div>
          <span class="eyebrow">${playable ? "Clip playback" : "Source required"}</span>
          <strong>${esc(candidateDisplayTitle(candidate))}</strong>
        </div>
        <div class="builder-preview-modes">
          <button class="active" type="button" data-builder-command="save">Reframed (${esc(edit.aspectRatio)})</button>
          <button type="button" data-builder-command="capture-frame" ${disabledAttr(!playable, "Attach playable source first")}>Capture frame</button>
        </div>
      </div>
      <div class="builder-preview-stage">
        <div class="builder-canvas-frame builder-preview-frame ${safeClass}" style="--preview-scale:${scale};--preview-x:${translateX}px;--preview-y:${translateY}px;--preview-rotate:${rotate}deg;--bg-strength:${Number(edit.background?.strength || 0)}px;">
          ${playable ? `<video class="builder-preview-bg" src="${esc(src)}" muted playsinline preload="metadata" data-start="${esc(start)}"></video>` : ""}
          <div class="builder-safe-guide"></div>
          ${source?.provenance === "DEMO_SOURCE" ? `<span class="builder-source-badge">Practice</span>` : ""}
        ${playable ? `
          <video
            class="studio-player builder-canvas-video"
            src="${esc(src)}"
            poster="${esc(poster)}"
            controls
            playsinline
            ...
            data-window-duration="${esc(duration)}"
          ></video>
          ${renderBuilderPreviewOverlays(candidate)}
        ` : `
          ${poster ? `<img class="builder-canvas-poster" src="${esc(poster)}" alt="">` : ""}
          <div class="builder-canvas-empty">
            <strong>No playable source attached</strong>
            <small>Capture or upload the real clip source before exporting.</small>
            <button data-nav-jump="radar">Back to Radar</button>
          </div>
        `}
        </div>
      </div>
      <div class="builder-canvas-footer builder-preview-controls">
        <button type="button" data-builder-command="split" ${disabledAttr(!candidate, "Select a clip first")}>Split</button>
        <button type="button" data-builder-command="mark-start" ${disabledAttr(!playable, "Attach playable source first")}>Set start</button>
        <button type="button" data-builder-command="mark-end" ${disabledAttr(!playable, "Attach playable source first")}>Set end</button>
        <button type="button" data-builder-command="captions">Captions</button>
        <button type="button" data-builder-command="create-package" ${disabledAttr(!source?.playable, "Attach playable source first")}>Send to package</button>
        <span>${esc(candidateWindowRangeLabel(candidate))}</span>
        <strong>${esc(duration)}s</strong>
      </div>
    </section>
  `;
}

function renderBuilderPreviewOverlays(candidate) {
  const edit = builderEditState();
  return `
    <div class="builder-overlay-layer">
      ${edit.captions?.enabled ? `<span class="builder-caption-overlay">${esc((edit.layers?.captions || ["THIS", "SHOT", "IS", "INSANE"]).slice(0, 4).join(" "))}</span>` : ""}
      ${(edit.layers?.stickers || []).slice(0, 2).map((item, index) => `<span class="builder-sticker-overlay sticker-${index}">${esc(item)}</span>`).join("")}
      ${edit.reframe?.showPath ? `<i class="builder-motion-path"></i>` : ""}
    </div>
  `;
}

function renderBuilderTimeline(source, candidate, candidates = []) {
  const duration = Math.max(
    1,
    sourceDurationSeconds(source),
    ...candidates.map((item) => candidateEndSeconds(item) || candidateWindowDuration(item)),
    candidate ? candidateEndSeconds(candidate) || candidateWindowDuration(candidate) : 0
  );
  const staged = candidates.length ? candidates : candidate ? [candidate] : [];
  const ticks = builderTimelineTicks(duration);
  const edit = builderEditState();
  const tracks = [
    { id: "video", label: "Video", blocks: staged.map((item, index) => ({ label: candidateDisplayTitle(item), start: candidateStartSeconds(item), duration: candidateWindowDuration(item), active: item.id === candidate?.id, thumb: studioCandidateThumb(item, index), candidateId: item.id })) },
    { id: "audio", label: "Audio", waveform: true, blocks: candidate ? [{ label: "Source audio", start: candidateStartSeconds(candidate), duration: candidateWindowDuration(candidate), active: true }] : [] },
    { id: "captions", label: "Captions", blocks: (edit.layers?.captions || []).slice(0, 8).map((label, index) => ({ label, start: candidateStartSeconds(candidate) + index * 2.2, duration: 1.9 })) },
    { id: "stickers", label: "Stickers", blocks: (edit.layers?.stickers || []).map((label, index) => ({ label, start: candidateStartSeconds(candidate) + 5 + index * 4, duration: 3 })) },
    { id: "effects", label: "Effects", blocks: (edit.layers?.effects || []).map((label, index) => ({ label, start: candidateStartSeconds(candidate) + 2 + index * 5, duration: 2.6 })) },
    { id: "zoom", label: "Zoom", curve: true, blocks: edit.autoZoom?.enabled ? [{ label: "Punch zoom", start: candidateStartSeconds(candidate) + 6, duration: 5 }] : [] },
    { id: "motion", label: "Motion", curve: true, blocks: (edit.layers?.motion || []).map((label, index) => ({ label, start: candidateStartSeconds(candidate) + 4 + index * 4, duration: 4 })) },
    { id: "safezone", label: "Safe Zone", blocks: edit.aspectRatio ? [{ label: edit.aspectRatio, start: 0, duration }] : [] }
  ];
  return `
    <section class="builder-timeline builder-multitrack-timeline">
      <div class="builder-timeline-toolbar">
        <div>
          <button type="button" data-builder-command="split">Split</button>
          <button type="button" data-builder-command="delete-layer">Delete</button>
          <button type="button" data-builder-command="duplicate-layer">Duplicate</button>
          <button type="button" data-builder-command="add-motion">Motion</button>
          <button type="button" data-builder-command="captions">Captions</button>
        </div>
        <label class="builder-timeline-zoom">Zoom <input type="range" min="25" max="120" value="${esc(edit.timeline?.zoom || 68)}" data-builder-setting="timeline.zoom"></label>
      </div>
      <div class="builder-timeline-ruler">
        ${ticks.map((tick) => `<span style="left:${tick.left}%">${esc(tick.label)}</span>`).join("")}
        <i class="builder-playhead" style="left:${Math.min(96, Math.max(2, Number(edit.timeline?.playhead || 35)))}%"></i>
      </div>
      ${tracks.map((track) => renderBuilderTrack(track, duration, candidate)).join("")}
    </section>
  `;
}

function renderBuilderTrack(track, duration, candidate) {
  const settings = builderValueAt(`tracks.${track.id}`) || {};
  const hidden = settings.hidden;
  const locked = settings.locked;
  const solo = settings.solo;
  return `
    <div class="builder-track-row ${hidden ? "muted" : ""}">
      <div class="builder-track-controls">
        <b>${esc(track.label)}</b>
        <span>
          <button type="button" class="${hidden ? "active" : ""}" data-builder-command="toggle-track:${esc(track.id)}:hidden" title="Hide track">Hide</button>
          <button type="button" class="${locked ? "active" : ""}" data-builder-command="toggle-track:${esc(track.id)}:locked" title="Lock track">Lock</button>
          <button type="button" class="${solo ? "active" : ""}" data-builder-command="toggle-track:${esc(track.id)}:solo" title="Solo track">S</button>
        </span>
      </div>
      <div class="builder-track-lane ${track.curve ? "curve" : ""} ${track.waveform ? "waveform" : ""}">
        ${track.waveform ? `<div class="builder-audio-wave">${Array.from({ length: 72 }, (_, index) => `<i style="height:${22 + ((index * 13) % 38)}%"></i>`).join("")}</div>` : ""}
        ${track.curve ? `<svg class="builder-motion-curve" viewBox="0 0 100 30" preserveAspectRatio="none"><path d="M0 21 C18 18 24 9 38 13 S68 24 82 15 S94 10 100 16"/></svg>` : ""}
        ${(track.blocks || []).map((item, index) => {
            const start = Math.max(0, item.start || 0);
            const length = Math.max(1, item.duration || 1);
            const left = Math.min(96, (start / duration) * 100);
            const width = Math.max(6, Math.min(100 - left, (length / duration) * 100));
            return `
              <button class="builder-clip-block builder-layer-block ${item.thumb ? "" : "no-thumb"} ${item.active ? "active" : ""}" style="left:${left}%;width:${width}%" ${item.candidateId ? `data-studio-select-candidate="${esc(item.candidateId)}"` : `data-builder-command="add-motion"`} title="${esc(item.label)}">
                ${item.thumb ? `<img src="${esc(item.thumb)}" alt="" loading="lazy">` : ""}
                <span>${esc(item.label)}</span>
              </button>
            `;
          }).join("") || `<span class="builder-track-empty">No ${esc(track.label.toLowerCase())} layers</span>`}
      </div>
    </div>
  `;
}

function builderTimelineTicks(duration) {
  const count = Math.min(8, Math.max(3, Math.ceil(duration / 3) + 1));
  return Array.from({ length: count }, (_, index) => {
    const value = count === 1 ? 0 : Math.round((duration / (count - 1)) * index);
    return {
...
function renderBuilderInspector(source, candidate, activeJob, renderedArtifact) {
  if (!candidate) {
    return `
      <div class="builder-inspector-empty">
        <h3>Builder</h3>
        <button class="primary" data-nav-jump="radar">Open Radar</button>
      </div>
    `;
  }
  ...
  const tab = state.builderInspectorTab || "inspector";
  const edit = builderEditState();
  const tabs = [
    ["inspector", "Inspector"],
    ["notes", "AI Notes"],
  ...
  const inspectorPanel = `
    <div class="builder-inspector-scroll">
      <section class="builder-inspector-section">
        <h3>Transform</h3>
        ${builderSlider("Scale", "transform.scale", 50, 180)}
        ${builderSlider("Position X", "transform.x", -120, 120, "px")}
        ${builderSlider("Position Y", "transform.y", -120, 120, "px")}
        ${builderSlider("Rotate", "transform.rotate", -30, 30, "deg")}
      </section>
      <section class="builder-inspector-section">
        <h3>Auto Reframe</h3>
        ${builderToggle("Enabled", "reframe.enabled", "Saved to project edit graph")}
        ${builderSelect("Tracking", "reframe.target", ["Smart AI", "Face", "Crosshair / player", "Mouse", "Largest Motion"])}
        ${builderSlider("Sensitivity", "reframe.sensitivity", 0, 100)}
        ${builderSlider("Smoothness", "reframe.smoothness", 0, 100)}
        ${builderToggle("Show crop path", "reframe.showPath", "Preview tracking path")}
      </section>
      <section class="builder-inspector-section">
        <h3>Background</h3>
        ${builderSelect("Style", "background.mode", ["blur", "heavy blur", "soft gradient", "mirror", "dark", "glow"])}
        ${builderSlider("Strength", "background.strength", 0, 100)}
      </section>
      <section class="builder-inspector-section builder-export-presets">
        <h3>Export Presets</h3>
        <button type="button" data-builder-aspect="9:16">TikTok / 1080x1920</button>
        <button type="button" data-builder-aspect="9:16">YouTube Shorts / 1080x1920</button>
        <button type="button" data-builder-aspect="4:5">Instagram Reels / 1080x1350</button>
        <button type="button" data-builder-command="export" ${disabledAttr(!(readiness.canRender && !state.studioBusy), readiness.renderReasons?.[0] || "Render setup is not ready")}>Render MP4</button>
      </section>
    </div>
  `;
  const notesPanel = `
    <div class="builder-inspector-scroll">
      ...
      <section class="builder-inspector-section">
        <h3>Recommended Edit</h3>
        <p>${esc(source?.playable ? "Reframe to 9:16, keep captions enabled, add one punch zoom on the strongest reaction, then export a vertical draft." : "Attach playable source first. Do not export metadata-only windows.")}</p>
      </section>
      <section class="builder-inspector-section">
        <h3>Active Layers</h3>
        <p>${esc((edit.layers?.captions || []).length)} captions · ${esc((edit.layers?.stickers || []).length)} stickers · ${esc((edit.layers?.effects || []).length)} effects · ${esc((edit.layers?.motion || []).length)} motion keys</p>
      </section>
    </div>
  `;
  const scorePanel = `
    <div class="builder-inspector-scroll">
      <div class="builder-inspector-metrics">
        ${metricCard(score)}
        ${metricCard(hook)}
        ${metricCard(engagement)}
      </div>
      <section class="builder-inspector-section">
        <h3>Source Truth</h3>
        <p>${esc(source?.playable ? `Playable source attached: ${sourceTruthLabel(source)}.` : "No playable source attached. Export and final packaging remain blocked.")}</p>
      </section>
      <section class="builder-inspector-section">
        <h3>Render State</h3>
        <p>${esc(renderedUrl ? "A rendered draft exists for this project." : activeJob ? activeJob.currentStep || activeJob.status : "No rendered draft yet.")}</p>
        ${activeJob ? `<div class="progress"><span style="width:${Math.min(100, Number(activeJob.progress || 0))}%"></span></div>` : ""}
      </section>
    </div>
  `;
  return `
    <div class="builder-inspector-head">
      <div>
        <span class="eyebrow">Selected Clip</span>
        <h2>${esc(candidateDisplayTitle(candidate))}</h2>
        <p>${esc(candidateWindowRangeLabel(candidate))} · ${esc(candidateWindowDuration(candidate))}s</p>
      </div>
      ${score.real ? scoreRing(score.value) : ""}
    </div>
    <div class="builder-inspector-tabs">
      ${tabs.map(([id, label]) => `<button class="${tab === id ? "active" : ""}" type="button" data-builder-inspector-tab="${esc(id)}">${esc(label)}</button>`).join("")}
    </div>
    ${tab === "notes" ? notesPanel : tab === "score" ? scorePanel : inspectorPanel}
    ${renderBuilderCapcutAgent(candidate, renderedArtifact, readiness)}
    <div class="builder-action-stack">
      <button class="primary" type="button" data-builder-command="export" ${disabledAttr(!(readiness.canRender && !state.studioBusy), readiness.renderReasons?.[0] || "Render setup is not ready")}>${renderedUrl ? "Render again" : "Render draft"}</button>
      <button type="button" data-builder-command="create-package" ${readiness.canPackage ? "" : "disabled"}>${packageReady ? "Package ready" : "Create package"}</button>
      <button data-nav-jump="radar">Back to Radar</button>
    </div>
  `;
}
...
function studioCandidateThumb(candidate, index = 0) {
  if (candidate?.thumbnailUrl) return resolvedThumbnailUrl(candidate.thumbnailUrl);
  if (!candidate?.sourceId) return "";
  return appUrl(`/api/media/sources/${encodeURIComponent(candidate.sourceId)}/frame?candidateId=${encodeURIComponent(candidate?.id || "")}&v=${index}`);
}

function renderStudioStage(source, candidate, renderedArtifact) {
  if (!source?.playable) {
    if (candidate) return renderPendingBuilderStage(candidate);
    return `
      <div class="studio-empty-stage">
        <span class="eyebrow">No verified source</span>
        <h3>Start a Clip Project</h3>
        <p>No playable media source is selected. Add approved media or open a verified Clip Radar candidate.</p>
        <div class="empty-actions">
  ...
  return `
    <div class="studio-stage ${vertical ? "vertical-mode" : ""}">
      ...
      <video
        ...
        data-window-duration="${esc(candidateWindowDuration(candidate))}"
      ></video>
...
function renderPendingBuilderStage(candidate) {
  const streamer = state.streamers.find((item) => item.id === candidate.streamerId);
  const sourceUrl = streamerSourceUrl(streamer);
  const playback = candidatePlaybackState(candidate);
  return `
    <div class="studio-empty-stage builder-pending-stage">
      <div class="builder-pending-media">
        ${radarThumb(candidate, 2)}
      </div>
      <div>
        <span class="eyebrow">Builder review</span>
        <h3>${esc(candidateDisplayTitle(candidate))}</h3>
        <p>${esc(playback.playable ? "Playable source is attached. Review the cut, then render the package." : "This clip is staged. Open the live source or upload the matching video before rendering.")}</p>
        <div class="builder-stage-facts">
          <span><b>${esc(candidateWindowRangeLabel(candidate))}</b><em>Window</em></span>
          <span><b>${esc(candidateWindowDuration(candidate))}s</b><em>Length</em></span>
          <span><b>${esc(playback.playable ? "Ready" : "Needs video")}</b><em>Video</em></span>
        </div>
        <div class="empty-actions">
          <button class="primary" data-preview-candidate="${esc(candidate.id)}">Open Details</button>
          ${sourceUrl ? `<button data-open-streamer-source="${esc(streamer?.id || "")}">Open Live Source</button>` : ""}
          <button data-nav-jump="radar">Back to Radar</button>
        </div>
      </div>
    </div>
...
function renderStudioTransport(source, candidate, candidates = []) {
  ...
  const editable = Boolean(source?.playable && candidate);
  return `
    <div class="studio-transport">
      ...
      <button data-studio-action="mark-start" ${editable ? "" : "disabled"}>Set start</button>
      <button data-studio-action="mark-end" ${editable ? "" : "disabled"}>Set end</button>
      <button data-studio-action="capture-frame" ${editable ? "" : "disabled"}>Capture frame</button>
...
function renderStudioCandidateRail(candidates = [], selected) {
  ...
  return `
    <div class="studio-rail">
      <div class="studio-section-head">
        <span class="eyebrow">Builder Queue</span>
        <strong>${candidates.length} staged clip${candidates.length === 1 ? "" : "s"}</strong>
      </div>
      <div class="studio-candidate-strip">
        ${visible.map((candidate, index) => `
          <button class="studio-candidate-card ${candidate.id === selected?.id ? "active" : ""}" data-studio-select-candidate="${esc(candidate.id)}">
            <img src="${esc(studioCandidateThumb(candidate, index))}" alt="" loading="lazy">
            <span>${provenancePill(candidate.sourceProvenance || candidate.provenance || "UNAVAILABLE")}</span>
            <strong>${esc(candidateDisplayTitle(candidate))}</strong>
            <small>${esc(candidateWindowRangeLabel(candidate))} · ${esc(candidateWindowDuration(candidate))}s</small>
            <em>${candidate.builderDraft ? "In Builder" : candidateScoreValue(candidate) == null ? "Needs review" : `${candidateScoreValue(candidate)} score`}</em>
          </button>
        `).join("") || empty("No clips are staged in Builder yet. Send a good Radar clip here first.")}
        ${hidden ? `<span class="studio-rail-overflow">+${esc(hidden)} more staged</span>` : ""}
      </div>
    </div>
  `;
}
...
function renderQueue() {
  ...
  const visibleApprovals = realApprovals();
        ...
        <section class="panel queue-board">
          <div class="queue-filterbar">
            ...
            <select aria-label="All streamers">
              ...
              ${realStreamers().map((streamer) => `<option>${esc(streamer.displayName)}</option>`).join("")}
            </select>
    ...
    <tr class="${approval.id === selectedId ? "selected" : ""}">
      <td>
        <button class="gate-request-cell" data-select-approval="${approval.id}">
          ${approval.type === "streamer_permission" ? streamerAccessThumb(context.streamer, index) : queueThumb(context.draft || {}, context.candidate, index)}
          <span>
            <strong>${esc(context.title || approval.title)}</strong>
            <small>${esc(gateRequestSubtitle(approval, context))}</small>
          </span>
        </button>
      </td>
...
function gateRequestSubtitle(approval, context) {
  if (approval.type === "streamer_permission") {
    const streamer = context.streamer || {};
    const live = isStreamerLive(streamer) ? "Live now" : liveLabel(streamer.liveStatus);
    return `${platformName(streamer.platform)} access review · ${live}`;
  }
  if (approval.type === "posting_draft") return `Post to ${platformName(context.platform)}`;
  return approval.title;
}

function streamerAccessThumb(streamer, index = 0) {
  const live = isStreamerLive(streamer);
  return `
    <div class="gate-access-thumb avatar-${index % 6}">
      <span>${esc(initials(streamer?.displayName || "SC"))}</span>
      <b>${live ? "LIVE" : "ACCESS"}</b>
      <em>${esc(streamer?.platform || "review")}</em>
    </div>
  `;
}
...
function renderApprovalDetail(approval) {
  if (approval.type === "streamer_permission") return renderStreamerAccessApprovalDetail(approval);
  const context = approvalContext(approval);
...
function renderStreamerAccessApprovalDetail(approval) {
  const context = approvalContext(approval);
  const streamer = context.streamer || {};
  const quality = clipQualityForStreamer(streamer);
  const live = isStreamerLive(streamer);
  const source = liveSourceLabel(streamer);
  return `
    <section class="panel gate-detail streamer-access-detail">
      <div class="gate-detail-head">
        <button data-nav-jump="gate">Back to Human Gate</button>
        <div>
          <h2>Approve Streamer Access</h2>
          <p>${esc(streamer.displayName || approval.title)} · ${esc(streamer.platform || "stream")} · ${esc(live ? "live now" : liveLabel(streamer.liveStatus))}</p>
        </div>
        ${riskBadge(approval.riskLevel)}
      </div>

      <div class="gate-detail-grid access-grid">
        <section class="gate-preview-card access-preview-card">
          <h3>Access Preview</h3>
          <div class="access-avatar-panel">
            <span class="creator-avatar large">${esc(initials(streamer.displayName || "SC"))}</span>
            <strong>${esc(streamer.displayName || "Unknown streamer")}</strong>
            <small>${esc(streamer.channelId || streamer.channelUrl || "No channel ID")}</small>
            ${liveBadge(streamer)}
          </div>
          <div class="preview-meta">
            <span><b>Approval</b>${esc(streamer.permissionStatus || "pending")}</span>
            <span><b>Monitoring</b>${streamer.monitorEnabled ? "On" : "Off"}</span>
            <span><b>Source</b>${esc(source)}</span>
          </div>
        </section>
        ...
        <section class="gate-detail-card">
          <h3>Request Details</h3>
          <div class="detail-kv">
            <span>Streamer</span><b>${esc(streamer.displayName || approval.title)}</b>
            <span>Platform</span><b>${esc(platformName(streamer.platform))}</b>
            <span>Channel</span><b>${esc(streamer.channelId || streamer.channelUrl || "not set")}</b>
            <span>Requested By</span><b>${esc(approval.actor || "Agent 101")}</b>
            <span>Allowed Uses</span><b>${esc((streamer.allowedUse || approval.evidence?.allowedUse || ["clips"]).join(", "))}</b>
            <span>Live Status</span><b>${esc(live ? "Live now" : liveLabel(streamer.liveStatus))}</b>
          </div>
          <h3>What Approval Does</h3>
          <p class="gate-caption">Approves local monitoring and clip-window review for this streamer. It does not publish, post, spend money, change accounts, or claim external rights.</p>
        </section>

        <section class="gate-analysis-card">
          <h3>Access Analysis</h3>
          <div class="access-quality-meter">
            ${renderClipReadinessMeter(streamer)}
          </div>
          <div class="analysis-bars">
            ${analysisBar("Live Signal", live ? Math.max(70, quality.score) : 18)}
            ${analysisBar("Clip Potential", quality.score)}
            ${analysisBar("Permission Confidence", streamer.permissionStatus === "approved" ? 92 : 44)}
            ${analysisBar("External Risk", 100 - (approval.riskLevel === "high" ? 72 : 38))}
          </div>
          <p>Agent 101 can watch and create internal 30-second review windows after approval. Posting stays locked behind separate Human Gate approval.</p>
        </section>

        <section class="gate-risk-card">
          <h3>Risk Assessment</h3>
          ${riskItem("Content Use", "Review")}
          ${riskItem("Posting", "Blocked")}
          ${riskItem("Account/API Action", "Blocked")}
          ${riskItem("Local Monitoring", live ? "Low" : "Review")}
        </section>

        <section class="gate-approval-actions">
          <h3>Approval Actions</h3>
          <button class="primary" data-gate-approve="${approval.id}">Approve Streamer Access</button>
          <button data-gate-sendback="${approval.id}">Send Back</button>
          <button class="danger" data-gate-reject="${approval.id}">Reject</button>
          <textarea readonly>Add note about this decision...</textarea>
        </section>
      </div>
      ...
      <div class="gate-timeline">
        ${timelineStep("Streamer added for review", true)}
        ${timelineStep("Live/API status checked", Boolean(streamer.lastCheckedAt || streamer.liveStatus))}
        ${timelineStep("Access decision pending", true)}
        ${timelineStep("Monitoring starts from Watchlist", false)}
      </div>
    </section>
  `;
}
    ...
    <section class="integrations-page">
      <div class="integration-hero panel">
        <div>
          ...
          <h2>Integrations</h2>
          <p>Truthful runtime status for AI, provider APIs, CapCut automation, media tools, storage, and Human Gate. Secrets stay server-side.</p>
        </div>
...
function renderIntegrationCard(connector) {
  ...
  const details = [
    ...
    connector.missingConfig?.length ? ["Missing", connector.missingConfig.join(", "), "missing"] : null
  ].filter(Boolean);
  const capabilities = (connector.capabilities || []).slice(0, 4);
  return `
    <section class="panel integration-card integration-card-${esc(tone)}">
      ...
      ${details.length ? `<div class="integration-facts">${details.map(([label, value, kind]) => `<span class="${kind === "missing" ? "integration-fact-missing" : ""}"><small>${esc(label)}</small><b>${esc(value)}</b></span>`).join("")}</div>` : ""}
      ${capabilities.length ? `<div class="integration-tags">${capabilities.map((item) => `<em title="${esc(item)}">${esc(item)}</em>`).join("")}</div>` : ""}
    ...
    <div class="grid cols-2">
      <section class="panel">
        ...
        <div class="kv">
          ...
          <span>CapCut Agent</span><span>${state.browser?.enabled ? "enabled" : "disabled"}</span>
      ...
      <section class="panel">
        <div class="toolbar">
          ...
          <div class="actions">
            <button data-browser-action="reset-profile">Reset CapCut Session</button>
          </div>
        </div>
        <div class="kv">
          <span>Automation mode</span><span>${esc(state.browser?.mode || "headless_screenshot")}</span>
      ...
      <section class="smoke-modal" role="dialog" aria-modal="true" aria-label="System smoke test">
        <header>
          <div>
            ...
            <h2>Smoke Test</h2>
            <p>Real checks for the API, media tools, CapCut automation, CapCut DNS, and state storage. Warnings stay warnings.</p>
          </div>
    ...
    <aside class="browser-side browser-task-rail">
      <section class="panel browser-card browser-task-card">
        ...
        <div class="browser-task-hero">
          <strong>${esc(activeTask?.goal || handoff?.clipPackage?.title || "Automation runtime")}</strong>
          <p>${esc(currentStep)}</p>
        </div>
        <div class="kv">
  ...
  view.innerHTML = `
    <section class="browser-workspace browser-workspace-v2">
      <header class="browser-topbar">
        <div>
          <span class="eyebrow">Automation Runtime</span>
...
async function runWatch(streamerId = "") {
  const streamer = state.streamers.find((item) => item.id === streamerId) || selectedStreamer();
...
async function captureWatchSession(sessionId) {
  if (!sessionId) return;
  toast("Saving a 30s local clip buffer...", "info");
  const result = await api(`/api/watch-sessions/${encodeURIComponent(sessionId)}/capture`, {
    method: "POST",
    body: "{}"
  });
  const savedCandidate = (result.candidates || []).find((candidate) => candidate.sourceId === result.source?.id)
    || (result.candidates || []).find((candidate) => candidate.mediaPlayable || candidate.bufferStatus === "verified_media_window");
  if (result.source && savedCandidate?.id) {
    state.selectedCandidateId = savedCandidate.id;
    state.previewCandidateId = "";
    state.livePreviewStreamerId = "";
    localStorage.setItem("selectedCandidateId", savedCandidate.id);
  }
  toast(
    result.source ? "Saved a real 30s clip. Radar is refreshing." : result.recorder?.message || "Recorder is not ready yet.",
    result.source ? "good" : "bad"
  );
  await refresh();
  if (result.source && savedCandidate?.id) setView("radar");
}

async function approveMonitorAndWatch(streamerId) {
  const streamer = state.streamers.find((item) => item.id === streamerId);
  if (!streamer) return;
  const approving = streamer.permissionStatus !== "approved";
  const enabling = !streamer.monitorEnabled;
  if (approving || enabling) {
    await api(`/api/twitch/streamers/${streamerId}/approve`, { method: "PATCH" });
  }
  state.selectedStreamerId = streamerId;
  localStorage.setItem("selectedStreamerId", streamerId);
  await loadCore();
  const updated = state.streamers.find((item) => item.id === streamerId);
  if (!isStreamerLive(updated)) {
    await checkStreamer(streamerId);
    await loadCore();
  }
  await runWatch(streamerId);
  toast("Approved locally and started 30s clip watcher", "good");
  setView("radar");
}
...
async function loadAgent101Sessions() {
  const result = await api("/api/agent101/sessions");
  state.agent101Sessions = result.sessions || [];
  if (!state.activeAgent101SessionId && state.agent101Sessions[0]?.sessionId) {
    state.activeAgent101SessionId = state.agent101Sessions[0].sessionId;
    localStorage.setItem("activeAgent101SessionId", state.activeAgent101SessionId);
  }
  if (state.activeAgent101SessionId) {
    await openAgent101Session(state.activeAgent101SessionId, { renderAfter: false }).catch(() => {
      state.agent101Session = null;
    });
  }
}

function connectAgent101Stream(sessionId) {
  if (!sessionId) return;
  if (state.agent101EventSource) state.agent101EventSource.close();
  state.agent101Events = [];
  const source = new EventSource(appUrl(`/api/agent101/stream/${encodeURIComponent(sessionId)}`));
  state.agent101EventSource = source;
  const push = (event) => {
    const data = JSON.parse(event.data || "{}");
    state.agent101Events.push(data);
    state.agent101Events = state.agent101Events.slice(-40);
    if (state.view === "agent101") render();
  };
  ["connected", "run_started", "model_call", "tool_start", "tool_result", "tool_error", "approval_required", "run_completed", "run_failed"].forEach((name) => {
    source.addEventListener(name, push);
  });
  source.addEventListener("run_completed", async () => {
    state.agent101Busy = false;
    source.close();
    await loadAgent101Sessions();
    if (state.view === "agent101") render();
  });
  source.addEventListener("run_failed", async () => {
    state.agent101Busy = false;
    source.close();
    await loadAgent101Sessions();
    if (state.view === "agent101") render();
  ...
  source.onerror = () => {
    state.agent101EventSource?.close();
    state.agent101EventSource = null;
  };
}

function connectCapcutAgentStream(sessionId) {
  if (!sessionId) return;
  if (state.capcutAgentEventSource) state.capcutAgentEventSource.close();
  const source = new EventSource(appUrl(`/api/capcut/sessions/${encodeURIComponent(sessionId)}/events`));
  state.capcutAgentEventSource = source;
  const refreshSession = async () => {
    const result = await api("/api/capcut/sessions").catch(() => null);
    if (result?.sessions) {
      state.capcutAgentSessions = result.sessions;
      if (state.view === "agent101" || state.view === "builder") render();
    }
  };
  ["connected", "capcut_agent_step", "approval_required", "capcut_agent_error"].forEach((name) => {
    source.addEventListener(name, refreshSession);
  });
  source.onerror = () => {
    state.capcutAgentEventSource?.close();
    state.capcutAgentEventSource = null;
  };
}

async function runCapcutAgentForm(form) {
  const clipId = cleanCommand(form.elements.clip_id?.value);
  if (!clipId) return toast("Render a verified clip before using CapCut Agent.", "bad");
  const practiceControl = form.elements.practice_confirmed;
  const payload = {
    clip_id: clipId,
    rendered_artifact_id: cleanCommand(form.elements.rendered_artifact_id?.value),
    practice_confirmed: Boolean(practiceControl?.checked || practiceControl?.value === "true"),
    edit_spec: {
      aspectRatio: cleanCommand(form.elements.aspect_ratio?.value) || "9:16",
      platform: cleanCommand(form.elements.platform?.value) || "tiktok",
      autoCaptions: Boolean(form.elements.auto_captions?.checked),
      autoReframe: Boolean(form.elements.auto_reframe?.checked),
      hookText: cleanCommand(form.elements.hook_text?.value)
    }
  };
  state.capcutAgentBusy = true;
  render();
  try {
    const result = await api("/api/capcut/edit", {
      method: "POST",
      body: JSON.stringify(payload)
    });
    state.capcutAgentBusy = false;
    if (result.session?.sessionId) {
      state.activeCapcutAgentSessionId = result.session.sessionId;
      localStorage.setItem("activeCapcutAgentSessionId", state.activeCapcutAgentSessionId);
      connectCapcutAgentStream(result.session.sessionId);
    }
    await loadCore();
    toast(
      result.requiresApproval
        ? `${result.approvalType?.replaceAll("_", " ") || "Approval"} required in Human Gate.`
        : result.message || "CapCut Agent run updated.",
      result.requiresApproval ? "info" : result.error ? "bad" : "good"
    );
    renderNav();
    render();
  } catch (error) {
...
async function runAgent101StudioCommand(form) {
  const message = cleanCommand(form.elements.message?.value);
  if (!message) return toast("Tell Agent 101 what to build first.", "bad");
  const sessionId = state.activeAgent101SessionId || `agent101_session_${Date.now()}`;
  state.activeAgent101SessionId = sessionId;
  localStorage.setItem("activeAgent101SessionId", sessionId);
  state.agent101Busy = true;
  state.agent101Events = [{ type: "queued", message: "Agent 101 request queued.", createdAt: new Date().toISOString() }];
  connectAgent101Stream(sessionId);
  form.reset();
  render();
  try {
    const result = await api("/api/agent101/run", {
      ...
      body: JSON.stringify({
        ...
        sessionId,
        agentMode: "studio",
        context: { agent101Studio: true }
      })
    });
    state.agent101Busy = false;
    state.activeAgent101SessionId = result.sessionId || sessionId;
    localStorage.setItem("activeAgent101SessionId", state.activeAgent101SessionId);
    toast(result.status === "FAILED" ? result.response : "Agent 101 finished the run.", result.status === "FAILED" ? "bad" : "good");
    await loadAgent101Sessions();
    renderNav();
    render();
  } catch (error) {
...
async function openAgent101Session(sessionId, options = {}) {
  state.activeAgent101SessionId = sessionId;
  localStorage.setItem("activeAgent101SessionId", sessionId);
  const result = await api(`/api/agent101/sessions/${encodeURIComponent(sessionId)}`);
  state.agent101Session = result;
  if (options.renderAfter !== false && state.view === "agent101") render();
}

async function viewAgent101File(pathValue) {
  const result = await api(`/api/agent101/files?path=${encodeURIComponent(pathValue)}`);
  state.agent101FileModal = result;
  render();
}
...
async function ensureBrowserSession() {
  ...
  const result = await api("/api/browser/sessions", {
    method: "POST",
    body: JSON.stringify({ purpose: "Operator automation runtime" })
  });
  ...
  try {
    if (action === "start-session") {
      await api("/api/browser/sessions", {
        method: "POST",
        body: JSON.stringify({ purpose: "StreamClipper CapCut automation" })
      });
      toast("CapCut automation session started", "good");
...
async function scoutStreamers() {
  const platform = state.scoutPlatform || document.querySelector("#scout-platform")?.value || "all";
  const result = await api(`/api/streamers/recommendations?platform=${encodeURIComponent(platform)}&limit=8`);
  state.recommendations = result.recommendations || [];
  state.manualReviewRecommendations = result.manualReviewRecommendations || [];
  state.recommendationsMessage = result.message || "";
  state.recommendationsMeta = {
    fallbackUsed: Boolean(result.fallbackUsed),
    providerBlocked: Boolean(result.providerBlocked),
    providers: result.providers || {},
    errors: result.errors || []
  };
  toast(
    state.recommendations.length
      ? `Agent 101 found ${state.recommendations.length} verified live streamers`
      : "No verified live streamer data returned yet",
    state.recommendations.length ? "good" : "warn"
  );
  render();
}
...
async function addRecommendedStreamer(index) {
  ...
  if (!item) return;
  const verifiedLive = Boolean(item.liveVerified && item.liveStatus === "live" && Number(item.viewerCount || 0) > 0);
  const liveStatus = verifiedLive ? "live" : (item.liveStatus || "unverified");
  const liveStatusReason = verifiedLive
    ? `Official ${item.platform || "provider"} API recommendation verified this channel as live.`
    : "Agent 101 scout recommendation only. Live status is not verified, so monitoring stays paused until an official check succeeds.";
  const result = await api("/api/twitch/streamers", {
    ...
    body: JSON.stringify({
      ...
      notes: `Added from Agent 101 Streamer Scout for Human Gate review. ${liveStatusReason} ${item.reason || ""}`.trim()
    })
  });
  ...
  toast(`${item.displayName} added for review. Monitoring is paused until approval and a live check pass.`, "good");
  state.recommendations = state.recommendations.filter((_, itemIndex) => itemIndex !== Number(index));
  await refresh();
}

async function removeSelectedStreamers() {
  const ids = [...new Set(state.selectedStreamerIds)].filter((id) => state.streamers.some((streamer) => streamer.id === id));
  if (!ids.length) return;
  const label = ids.length === 1
    ? state.streamers.find((streamer) => streamer.id === ids[0])?.displayName || "this streamer"
    : `${ids.length} streamers`;
  if (!window.confirm(`Remove ${label} from the watchlist? This does not touch any external account.`)) return;
  for (const id of ids) {
    await api(`/api/twitch/streamers/${id}`, { method: "DELETE" });
  }
  state.selectedStreamerIds = [];
  if (ids.includes(state.selectedStreamerId)) {
    state.selectedStreamerId = "";
    localStorage.removeItem("selectedStreamerId");
  }
...
async function deleteCandidate(candidateId, options = {}) {
  ...
  if (!candidate) return;
  const label = candidateDisplayTitle(candidate);
  const stopWatcher = Boolean(options.stopWatcher);
  const stopCopy = stopWatcher
    ? " This will also stop/pause the source watcher so the same stream does not refill Clip Radar."
    : "";
  if (!window.confirm(`Delete ${label}? This removes the local Radar candidate and does not touch Twitch or any external account.${stopCopy}`)) return;
  const result = await api(`/api/clip-candidates/${encodeURIComponent(candidateId)}${stopWatcher ? "?stopWatcher=1" : ""}`, { method: "DELETE" });
  ...
  toast(result.stoppedWatchSession ? "Clip deleted and watcher stopped" : "Clip candidate deleted", "info");
  await refresh();
}

async function bulkDeleteCandidates(ids, label, options = {}) {
  const uniqueIds = [...new Set(ids)].filter((id) => state.candidates.some((candidate) => candidate.id === id));
  if (!uniqueIds.length) return;
  const stopWatchers = Boolean(options.stopWatchers);
  const stopCopy = stopWatchers
    ? " This will also stop/pause any source watcher that is refilling source-pending windows."
    : "";
  if (!window.confirm(`Delete ${label}? This removes local Radar candidates and does not touch Twitch or any external account.${stopCopy}`)) return;
  const result = await api("/api/clip-candidates/bulk-delete", {
    method: "POST",
    body: JSON.stringify({ ids: uniqueIds, stopWatchers })
  });
  const deletedIds = result.deleted || [];
  state.candidates = state.candidates.filter((candidate) => !deletedIds.includes(candidate.id));
  state.selectedCandidateIds = state.selectedCandidateIds.filter((id) => !deletedIds.includes(id));
  if (deletedIds.includes(state.selectedCandidateId)) {
    state.selectedCandidateId = "";
    localStorage.removeItem("selectedCandidateId");
  }
  if (deletedIds.includes(state.previewCandidateId)) state.previewCandidateId = "";
  renderNav();
  render();
  const blocked = result.blocked?.length || 0;
  const stopped = result.stoppedWatchSessions?.length || 0;
  toast(
    blocked
      ? `Deleted ${deletedIds.length}; ${blocked} blocked`
      : stopped
        ? `Deleted ${deletedIds.length} and stopped ${stopped} watcher${stopped === 1 ? "" : "s"}`
        : `Deleted ${deletedIds.length} clip candidates`,
    blocked ? "warn" : "info"
  );
  await refresh();
}

async function deleteSelectedCandidates(options = {}) {
  await bulkDeleteCandidates(state.selectedCandidateIds, `${state.selectedCandidateIds.length} selected clip candidate${state.selectedCandidateIds.length === 1 ? "" : "s"}`, options);
}

async function deleteVisibleCandidates(options = {}) {
  const { sorted } = radarCandidateScope();
  await bulkDeleteCandidates(sorted.map((candidate) => candidate.id), `all ${sorted.length} visible clip candidate${sorted.length === 1 ? "" : "s"}`, options);
}
...
async function saveBuilderDraft(id, options = {}) {
  state.selectedCandidateId = id;
  localStorage.setItem("selectedCandidateId", id);
  ...
  if (!options.silent) toast("Clip builder draft saved", "good");
  await refresh();
  setView("builder");
}

async function sendCandidateToBuilder(id) {
  ...
  if (!playback.playable) {
    const session = (state.activeWatchSessions || []).find((item) => item.id === candidate.watchSessionId);
    if (session?.id) {
      await captureWatchSession(session.id);
      return;
    }
    toast("Attach or upload a playable source before sending this to Builder.", "bad");
    return;
  }
  await saveBuilderDraft(id, { silent: true });
  toast("Opened clip in Builder", "good");
}
...
async function gate(path, id) {
  ...
  if (result.watchSession?.id) {
    state.selectedWatchSessionId = result.watchSession.id;
    localStorage.setItem("selectedWatchSessionId", result.watchSession.id);
    toast("Human Gate approved. Live 30s watcher started.", "good");
    await loadCore();
    setView("radar");
    return;
  }
...
document.addEventListener("submit", async (event) => {
  ...
  const capcutAgentForm = event.target.closest("#agent101-capcut-form, #builder-capcut-agent-form");
  ...
  if (!agentForm && !capcutAgentForm && !browserForm && !browserTypeForm && !mediaUploadForm && !form) return;
  event.preventDefault();
  try {
    if (capcutAgentForm) {
      await runCapcutAgentForm(capcutAgentForm);
      return;
    }
    if (agentForm) {
...
document.addEventListener("change", (event) => {
  const builderSetting = event.target.closest("[data-builder-setting]");
  if (builderSetting) {
    updateBuilderSetting(builderSetting.dataset.builderSetting, coerceBuilderInput(builderSetting));
    render();
    return;
  }
  const scoutPlatform = event.target.closest("[data-scout-platform]");
  if (scoutPlatform) {
    state.scoutPlatform = scoutPlatform.value || "all";
    localStorage.setItem("scoutPlatform", state.scoutPlatform);
    state.recommendations = [];
    state.recommendationsMessage = "Scout source changed. Press Scout streamers to run the selected provider.";
    state.recommendationsMeta = null;
    return;
  }
});
...
document.addEventListener("input", (event) => {
  const builderSetting = event.target.closest("[data-builder-setting]");
  if (!builderSetting) return;
  updateBuilderSetting(builderSetting.dataset.builderSetting, coerceBuilderInput(builderSetting));
  window.clearTimeout(document.builderInputRenderTimer);
  document.builderInputRenderTimer = window.setTimeout(render, 80);
});
...
document.addEventListener("click", async (event) => {
  ...
  const capcutAgentSessionId = target.closest("[data-capcut-agent-session]")?.dataset.capcutAgentSession;
  const action = target.closest("[data-action]")?.dataset.action;
  const watchlistFilter = target.closest("[data-watchlist-filter]")?.dataset.watchlistFilter;
  ...
  const captureSessionId = target.closest("[data-capture-session]")?.dataset.captureSession;
  ...
  const builderTool = target.closest("[data-builder-tool]")?.dataset.builderTool;
  const builderCommand = target.closest("[data-builder-command]")?.dataset.builderCommand;
  const builderPreset = target.closest("[data-builder-preset]")?.dataset.builderPreset;
  const builderAspect = target.closest("[data-builder-aspect]")?.dataset.builderAspect;
  const builderInspectorTab = target.closest("[data-builder-inspector-tab]")?.dataset.builderInspectorTab;
  ...
  const livePreviewTrigger = target.closest("[data-live-preview-streamer]");
  const livePreviewStreamer = livePreviewTrigger?.dataset.livePreviewStreamer;
  const closeLivePreview = target.closest("[data-close-live-preview]");
  const previewOpenBuilder = target.closest("[data-preview-open-builder]")?.dataset.previewOpenBuilder;
  const builderCandidateId = target.closest("[data-builder-candidate]")?.dataset.builderCandidate;
  const openStreamerSourceId = target.closest("[data-open-streamer-source]")?.dataset.openStreamerSource;
  const selectCandidate = target.closest("[data-select-candidate]")?.dataset.selectCandidate;
  const candidateCheckId = target.closest("[data-candidate-check]")?.dataset.candidateCheck;
  const candidateCheckAll = target.closest("[data-candidate-check-all]");
  ...
  const deleteCandidateId = target.closest("[data-delete-candidate]")?.dataset.deleteCandidate;
  const deleteCandidateStopId = target.closest("[data-delete-candidate-stop]")?.dataset.deleteCandidateStop;
  ...
  const streamerCheckId = target.closest("[data-streamer-check]")?.dataset.streamerCheck;
  ...
  if (capcutAgentSessionId) {
    state.activeCapcutAgentSessionId = capcutAgentSessionId;
    localStorage.setItem("activeCapcutAgentSessionId", capcutAgentSessionId);
    connectCapcutAgentStream(capcutAgentSessionId);
    render();
    return;
  }
  ...
  try {
    ...
    if (watchlistFilter) {
      state.watchlistFilter = watchlistFilter;
      localStorage.setItem("watchlistFilter", watchlistFilter);
      state.selectedStreamerIds = [];
      render();
      return;
    }
    ...
    if (captureSessionId) {
      await captureWatchSession(captureSessionId);
      return;
    }
    if (browserTab) {
      const active = state.browser?.activeSession || (state.browser?.sessions || []).find((session) => session.status !== "closed");
    ...
    if (builderTool) {
      setBuilderTool(builderTool);
      return;
    }
    if (builderInspectorTab) {
      setBuilderInspectorTab(builderInspectorTab);
      return;
    }
    if (builderPreset) {
      applyBuilderPreset(builderPreset);
      return;
    }
    if (builderAspect) {
      updateBuilderSetting("aspectRatio", builderAspect);
      render();
      return;
    }
    if (builderCommand) {
      await runBuilderCommand(builderCommand);
      return;
    }
    if (studioCandidate) {
    ...
    if (previewOpenBuilder) {
      state.previewCandidateId = "";
      await sendCandidateToBuilder(previewOpenBuilder);
      return;
    }
    if (builderCandidateId) {
      await sendCandidateToBuilder(builderCandidateId);
      return;
    }
    if (openStreamerSourceId) {
      const streamer = state.streamers.find((item) => item.id === openStreamerSourceId);
      const url = streamerSourceUrl(streamer);
    ...
    if (livePreviewStreamer) {
      ...
      if (interactive && !interactive.hasAttribute("data-live-preview-streamer")) {
        // Let the button-specific handler below run instead of opening the card.
      } else {
        state.selectedStreamerId = livePreviewStreamer;
        state.livePreviewStreamerId = livePreviewStreamer;
        state.previewCandidateId = "";
        localStorage.setItem("selectedStreamerId", livePreviewStreamer);
        render();
        return;
      }
    }
    ...
    if (streamerCheckId) {
      const selected = new Set(state.selectedStreamerIds);
      if (target.checked) selected.add(streamerCheckId);
      else selected.delete(streamerCheckId);
      state.selectedStreamerIds = [...selected];
      render();
      return;
    }
    if (selectStreamer) {
    ...
    if (action === "remove-selected-streamers") await removeSelectedStreamers();
    if (action === "delete-selected-candidates") {
      await deleteSelectedCandidates();
      return;
    }
    if (action === "delete-selected-stop-watchers") {
      await deleteSelectedCandidates({ stopWatchers: true });
      return;
    }
    if (action === "delete-visible-candidates") {
      await deleteVisibleCandidates();
      return;
    }
    if (action === "delete-visible-stop-watchers") {
      await deleteVisibleCandidates({ stopWatchers: true });
      return;
    }
    if (action === "select-visible-candidates") {
      const { sorted } = radarCandidateScope();
      state.selectedCandidateIds = sorted.map((candidate) => candidate.id);
      if (!state.selectedCandidateId && sorted[0]?.id) {
        state.selectedCandidateId = sorted[0].id;
        localStorage.setItem("selectedCandidateId", sorted[0].id);
      }
      render();
      return;
    }
    if (action === "clear-selected-candidates") {
    ...
    if (candidateCheckId !== undefined) {
      const selected = new Set(state.selectedCandidateIds);
      if (target.checked) selected.add(candidateCheckId);
      else selected.delete(candidateCheckId);
      state.selectedCandidateIds = [...selected];
      state.selectedCandidateId = candidateCheckId;
      localStorage.setItem("selectedCandidateId", candidateCheckId);
      render();
      return;
    }
    if (candidateCheckAll) {
      const { sorted } = radarCandidateScope();
      const selected = new Set(state.selectedCandidateIds);
      if (target.checked) sorted.forEach((candidate) => selected.add(candidate.id));
      else sorted.forEach((candidate) => selected.delete(candidate.id));
      state.selectedCandidateIds = [...selected];
      render();
      return;
    }
    ...
    if (deleteCandidateStopId) {
      await deleteCandidate(deleteCandidateStopId, { stopWatcher: true });
      return;
    }
    if (toggleId) {
      const streamer = state.streamers.find((item) => item.id === toggleId);
      if (streamer && !streamer.monitorEnabled && isStreamerLive(streamer)) {
        await approveMonitorAndWatch(toggleId);
        return;
      }
      ...
      toast(streamer.monitorEnabled ? "Monitoring paused" : "Monitoring enabled", "good");
      await refresh();
    }
    if (approveStreamerId) {
      const streamer = state.streamers.find((item) => item.id === approveStreamerId);
      if (streamer && isStreamerLive(streamer)) {
        await approveMonitorAndWatch(approveStreamerId);
        return;
      }
      await api(`/api/twitch/streamers/${approveStreamerId}/approve`, { method: "PATCH" });
      toast("Streamer approved and saved", "good");
      await refresh();
    }
    if (checkStreamerId) await checkStreamer(checkStreamerId);
    if (deleteId) {
      await api(`/api/twitch/streamers/${deleteId}`, { method: "DELETE" });
      state.selectedStreamerIds = state.selectedStreamerIds.filter((id) => id !== deleteId);
      if (state.selectedStreamerId === deleteId) {
        state.selectedStreamerId = "";
        localStorage.removeItem("selectedStreamerId");
      }
