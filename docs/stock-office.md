...
`services/stock-intelligence-scheduler.js` keeps the evaluator, bounded online research, and copy plan current while Argentum is open, even when the Stock Office page is not visible. Its restart-safe state is stored with mode `0600` in the local Argentum application-support directory. Startup is immediate and each successful exchange batch hands off to the next after a short event-loop yield; failed batches retry safely instead of spinning. Argentum rotates bounded 200-symbol batches through the available NASDAQ/NYSE common-stock catalog and reports both current-batch and full-sweep coverage. Lightweight proposal/broker readiness is recomputed once per second from loaded state, while live Robinhood reads remain bounded by the connector's separate five-second cache. Official Form 4 attempts are limited to hourly, delayed Form 13F research attempts to daily, and structured news/profile context to every 30 minutes.
...
Optional cadence environment variables are `STOCK_GURU_AUTO_REFRESH_ACTIVE_MINUTES`, `STOCK_GURU_READINESS_INTERVAL_MS`, `STOCK_GURU_AUTO_REFRESH_PREMARKET_MINUTES`, `STOCK_GURU_AUTO_REFRESH_AFTER_HOURS_MINUTES`, `STOCK_GURU_AUTO_REFRESH_OVERNIGHT_MINUTES`, `STOCK_GURU_AUTO_REFRESH_WEEKEND_MINUTES`, `STOCK_GURU_AUTO_FORM4_MINUTES`, `STOCK_GURU_AUTO_13F_MINUTES`, `STOCK_GURU_AUTO_NEWS_MINUTES`, and `STOCK_GURU_AUTO_REFRESH_STARTUP_DELAY_MS`. Set `STOCK_GURU_AUTO_REFRESH_DISABLED=1` to keep only manual refreshes. Faster UI/readiness evaluation does not manufacture newer quotes; every executable proposal still requires provider freshness and a fresh official broker quote.
...
Supported commands are `/status`, `/portfolio`, `/positions`, `/watchlist`, `/opportunities`, `/pending`, `/research SYMBOL`, `/overnight`, `/morning`, `/mirror`, `/sources`, `/risk`, `/performance`, `/health`, `/symbol SYMBOL`, and `/help`.
...
- Continuous research is local-process continuous, not cloud continuous. It stops if Argentum or the Mac is stopped.
- Market data is polling-based. The one-second readiness loop does not turn a delayed/free provider into an exchange-grade real-time stream.
- Twelve Data, FMP, and Alpha Vantage require separately supplied server-side credentials. Yahoo/yfinance are fallbacks and may be delayed, incomplete, throttled, or unavailable.
- The current official SEC automation covers configured Form 4 identities and delayed Form 13F managers. It does not yet provide a complete 8-K, 10-Q, 10-K, 13D/13G, earnings-calendar, congressional-trade, social-signal, or options-intelligence feed.
- Form 13F remains research-only because it does not disclose exact trade time or execution price. Unmapped CUSIPs cannot become orders.
- Performance analytics separates measured signal outcomes, modeled simulation, paper results, and verified broker fills. Small samples display as insufficient; no parameter proposal activates itself.
- Portfolio risk enforces capital, order, symbol, position, sector, daily-loss, trade-count, buying-power, pending-order, stop-distance, and reward/risk constraints. It does not yet calculate a live return-correlation matrix across holdings.
- Equities are the verified order path. Options, crypto, event contracts, transfers, and account-setting changes are not enabled by Stock Office.
- A production rollout still requires healthy provider credentials, a real SEC contact identity for SEC jobs, authorized Telegram IDs if remote control is wanted, a verified dedicated Robinhood Agentic account, operator-selected limits, and a supervised no-order account/readiness check. No automated test should place a live order.
