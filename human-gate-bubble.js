  ...
  function render() {
    ...
    list.innerHTML = count
      ? requests.slice(0, 8).map((approval) => {
          ...
          const trade = approval.details?.tradeSummary;
          const tradeFacts = trade && trade.symbol ? `
            <div class="argentum-gate-trade-facts">
              <span><small>Order</small><strong>${escapeHtml(`${trade.side || ""} ${trade.symbol}`)}</strong></span>
              <span><small>Value</small><strong>${escapeHtml(Number.isFinite(Number(trade.estimatedOrderValue)) ? `$${Number(trade.estimatedOrderValue).toFixed(2)}` : "—")}</strong></span>
              <span><small>Shares</small><strong>${escapeHtml(Number.isFinite(Number(trade.quantity)) ? Number(trade.quantity).toFixed(6) : "—")}</strong></span>
              <span><small>Score</small><strong>${escapeHtml(Number.isFinite(Number(trade.score)) ? Math.round(Number(trade.score)) : "—")}</strong></span>
              <span><small>Confidence</small><strong>${escapeHtml(Number.isFinite(Number(trade.confidence)) ? Math.round(Number(trade.confidence)) : "—")}</strong></span>
              <span><small>Risk</small><strong>${escapeHtml(Number.isFinite(Number(trade.riskDollars)) ? `$${Number(trade.riskDollars).toFixed(2)}` : "—")}</strong></span>
              <span><small>Stop</small><strong>${escapeHtml(Number.isFinite(Number(trade.stop)) ? `$${Number(trade.stop).toFixed(2)}` : "—")}</strong></span>
              <span><small>Expires</small><strong>${escapeHtml(relativeTime(approval.expiresAt))}</strong></span>
            </div>` : "";
          return `
            <article class="argentum-gate-request ${escapeHtml(risk)}" data-gate-request="${escapeHtml(approval.id)}">
              ...
              ${tradeFacts}
              ${tradeFacts ? "" : `<p>${escapeHtml(approval.action || "Review the exact request before anything consequential continues.")}</p>`}
              <details>
