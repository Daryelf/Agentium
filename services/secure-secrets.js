...
function setSecret({ dataDir, provider, value, preferKeychain = true }) {
  if (preferKeychain && canUseKeychain()) {
    const saved = setKeychainSecret(provider, value);
    setFallbackSecret(dataDir, provider, value);
    return { ...saved, recoveryStorage: "encrypted_local_file" };
  }
  return setFallbackSecret(dataDir, provider, value);
}

function getSecret({ dataDir, provider, storage = "" }) {
  if (storage === "mac_keychain" || (!storage && canUseKeychain())) {
    return getKeychainSecret(provider) || getFallbackSecret(dataDir, provider);
  }
...
function deleteSecret({ dataDir, provider, storage = "" }) {
  if (storage === "mac_keychain" || (!storage && canUseKeychain())) {
    const removed = deleteKeychainSecret(provider);
    deleteFallbackSecret(dataDir, provider);
    return removed;
  }
