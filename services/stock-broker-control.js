...
function parsedRewardRisk(value) {
...
function normalizeGuardrails(value = {}) {
  ...
  return {
    ...
    maxPortfolioExposurePct: clamp(value.maxPortfolioExposurePct ?? value.max_portfolio_exposure_pct, 0.01, 1, 1),
    maxSinglePositionPct: clamp(value.maxSinglePositionPct ?? value.max_single_position_pct, 0.01, 1, 0.2),
    maxSectorExposurePct: clamp(value.maxSectorExposurePct ?? value.max_sector_exposure_pct, 0.01, 1, 0.4),
    maxCorrelatedExposurePct: clamp(value.maxCorrelatedExposurePct ?? value.max_correlated_exposure_pct, 0.01, 1, 0.5),
    minimumRewardRisk: clamp(value.minimumRewardRisk ?? value.minimum_reward_risk, 0.25, 20, 1.5),
...
function normalizeTradeDraft(draft = {}) {
  ...
  const status = ["blocked", "proposed", "ready_for_broker_review", "awaiting_human_gate", "approved", "dispatch_claimed", "submitting", "submitted", "partially_filled", "cancel_requested", "review_rejected", "reconciliation_required", "unknown_reconciling", "expired", "cancelled", "dispatched", "filled", "rejected"].includes(draft.status)
    ? draft.status
    : "blocked";
  return {
    ...
    riskDecision: draft.riskDecision && typeof draft.riskDecision === "object" && !Array.isArray(draft.riskDecision) ? draft.riskDecision : null,
    tradePlan: draft.tradePlan && typeof draft.tradePlan === "object" && !Array.isArray(draft.tradePlan) ? draft.tradePlan : null,
...
function buildTradeDraft(input = {}, snapshot = {}, options = {}) {
  ...
  const opportunity = (snapshot.intelligence?.opportunities || []).find((item) => normalizeSymbol(item.symbol) === symbol) || null;
  const researchItem = (snapshot.research?.tickers || []).find((item) => normalizeSymbol(item.ticker || item.symbol) === symbol) || null;
  const sector = String(researchItem?.sector || opportunity?.company?.sector || "UNKNOWN").slice(0, 100);
  ...
  const sectorBySymbol = Object.fromEntries((snapshot.research?.tickers || []).map((item) => [normalizeSymbol(item.ticker || item.symbol), String(item.sector || "UNKNOWN")]));
  const portfolioRisk = portfolioRiskState({
    accountEquity: control.accountValueDollars,
    buyingPower: control.buyingPowerDollars,
    principal: guardrails.principalDollars,
    positions: capital.positionValues.map((item) => ({ ...item, sector: sectorBySymbol[item.symbol] || "UNKNOWN" })),
    pendingOrders: (Array.isArray(snapshot.broker?.openOrders) ? snapshot.broker.openOrders : []).filter(orderIsActive).map((order) => ({
      symbol: normalizeSymbol(order.symbol),
      side: String(order.side || "").toUpperCase(),
      notional: orderNotional(order, Object.fromEntries(capital.positionValues.map((item) => [item.symbol, item.currentPrice]))),
      sector: sectorBySymbol[normalizeSymbol(order.symbol)] || "UNKNOWN",
    })),
    limits: guardrails,
  });
  ...
  const sizing = side === "BUY" ? sizePosition({
    ...
    stop: stopLoss,
    requestedDollars,
    currentPositionDollars: symbolPositionValue + pendingSymbolBuyDollars,
  ...
  if (side === "BUY") {
    ...
    addCheck(checks, blockers, "reward_risk", evaluatedRewardRisk !== null && evaluatedRewardRisk >= guardrails.minimumRewardRisk, evaluatedRewardRisk === null
      ? "BUY requires a target supported by strategy logic so reward/risk can be verified."
      : `Reward/risk ${evaluatedRewardRisk.toFixed(2)} is below the configured ${guardrails.minimumRewardRisk.toFixed(2)} minimum.`);
    addCheck(checks, blockers, "portfolio_risk", sizing?.eligible === true && requestedDollars <= finiteNumber(sizing?.permittedDollars, 0) + 0.01, sizing?.blockers?.[0]
      || `The centralized portfolio-risk engine permits at most $${finiteNumber(sizing?.permittedDollars, 0).toFixed(2)} for this exact setup.`);
  } else {
  ...
  const tradePlan = side === "BUY" ? buildTradePlan({
    symbol,
    direction: "LONG",
    entry: referencePrice,
    entryZone: record?.entryZone,
    invalidation: record?.invalidationRule,
    stop: stopLoss,
    target1,
    target2,
    sizing,
    opportunityScore: opportunity?.overallScore ?? opportunity?.aiScore ?? record?.score,
    confidenceScore: opportunity?.confidenceScore ?? null,
    catalyst: opportunity?.company?.catalystSummary || null,
    reasons: [record?.mainReasonValid || record?.mainReason],
    risks: [record?.mainRisk, ...(opportunity?.blockers || []).map((item) => item.reason || item.code || item)],
  }) : null;
  const core = {
    symbol,
    side,
