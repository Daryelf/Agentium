...
function buildContinuousReviewView(input = {}, options = {}) {
  ...
  const proposals = (Array.isArray(plan.proposals) ? plan.proposals : []).map((proposal) => {
    ...
    return {
      ...
      reviewEnteredAt: draft?.createdAt
        || proposal.reviewEnteredAt
        || proposal.research?.firstSeenAt
        || proposal.research?.lastResearchedAt
        || plan.generatedAt
        || null,
  ...
  return {
    ...
    cycle: {
      ...
      decisionCadenceSeconds: Number(review.decisionCadenceSeconds || 1),
