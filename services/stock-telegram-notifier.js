...
const MAX_RECENT = 12;
const DELIVERY_RETRY_COOLDOWN_MS = 30_000;
const CONTROL_COMMANDS = ["status", "portfolio", "positions", "watchlist", "opportunities", "pending", "research", "overnight", "morning", "mirror", "sources", "risk", "performance", "health", "help", "symbol"];
...
function createStockTelegramNotifier(options = {}) {
  ...
  function publicStatus(approvals = []) {
    ...
    return {
      ...
      recoveryRequired: settings.enabled && (!token || !chatId),
  ...
  async function deliver({ eventId, kind, text, replyMarkup = null, chatId: requestedChatId = "" }, approvals = []) {
    ...
    const task = (async () => {
      ...
      if (!status.enabled) {
        const reason = status.state === "setup_required"
          ? "Telegram credentials are unavailable. Argentum will retry while the Human Gate request remains active."
          : "Telegram needs its approved destination enabled. Argentum will retry while the Human Gate request remains active.";
        return { sent: false, state: status.state, reason };
      }
      const settings = readSettings();
      if (settings.deliveredEventIds.includes(eventId)) return { sent: false, state: "duplicate", reason: "This event was already delivered." };
      const priorFailure = settings.recent.find((item) => item.id === eventId && item.status === "failed");
      const priorFailureAt = Date.parse(priorFailure?.sentAt || "");
      if (Number.isFinite(priorFailureAt) && nowFn().getTime() - priorFailureAt < DELIVERY_RETRY_COOLDOWN_MS) {
        return { sent: false, state: "retry_wait", reason: "Telegram delivery is waiting for the short retry cooldown." };
      }
      const { token, chatId } = credentials();
      try {
  ...
  async function notifyQualifiedProposal(proposal = {}, draft = {}, approval = {}, approvals = []) {
    ...
    const eventId = `qualified-proposal:${secretHash(approval.id).slice(0, 24)}`;
