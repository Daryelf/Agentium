...
Daily quantitative features are centralized under `stock_guru.quant`. The same deterministic snapshot now supplies the evaluator's moving averages, Wilder RSI/ATR, MACD, momentum acceleration, volatility, contextual volume, SPY/QQQ/sector relative strength, and support/resistance zones. Indicators with insufficient history remain `null`; they are not converted to zero or neutral values. A non-dry-run `evaluate` writes `data/quant_features.json` alongside `reports/evaluations.json` for numerical inspection.

The v2 context/scoring path adds regime, breadth, optional FRED rates, fundamentals, earnings risk, structured sentiment, delayed institutional context, liquidity, confidence, conflict penalties, portfolio-aware sizing, universe percentiles, multiple ranking views, and daily `analysis_snapshots`. The analytical object cannot bypass Human Gate.

Run the leakage-safe five-year validation with realistic liquidity costs and an unseen walk-forward period:

```bash
./bin/stock-guru quant-backtest
```

An install-ready macOS LaunchAgent template is available at `config/com.argentum.quant-engine.plist`. It runs the independent computation daemon under `caffeinate -s`, defaults to `OBSERVE`, uses an idempotent SQLite queue, and writes heartbeat/health/log artifacts. Copying/loading that plist is an explicit system-level operator step; it is not performed by repository tests.
...
Argentum Stock Office uses the same rotating catalog with `--max-symbols 200 --rotate-count 200`. That mode spends every batch slot advancing through the exchange catalog, persists the cursor across restarts, and starts the next bounded batch as soon as the previous one completes. It does not try to download thousands of symbols in one process.
