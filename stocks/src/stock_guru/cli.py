...
from .catalysts import build_catalyst_events
...
from .rates import fetch_macro_rate_context, market_regime_rate_values
from .market import market_state
from .multi_timeframe import build_multi_timeframe_contexts, write_intraday_context_report
from .market_context import CONTEXT_BENCHMARKS, SECTOR_ETFS, build_market_regime, write_market_context_report
...
from .quant.context import build_symbol_context
from .quant.engine import build_quant_snapshots, write_quant_report
from .quant.ranking import build_analysis_object, rank_analysis_objects, write_analysis_report
from .quant.scoring import score_symbol
from .quant.validation import run_quant_backtest, walk_forward_validate, write_quant_backtest_report
from .research import fetch_equity_research, load_research_json, money as research_money, pct as research_pct, ratio as research_ratio, write_research_json, write_research_report
...
from .universe import DynamicUniverse, build_dynamic_universe, commit_dynamic_universe
...
def write_argentum_research_progress(
    phase: str,
    symbols: list[str],
    *,
    current_ticker: str = "",
    ...
    progress_file = os.environ.get("STOCK_GURU_PROGRESS_FILE", "").strip()
    run_id = os.environ.get("STOCK_GURU_PROGRESS_RUN_ID", "").strip()
    if not progress_file or not run_id:
        return
    path = Path(progress_file).expanduser().resolve()
    total = len(symbols)
    bounded_completed = max(0, min(total, int(completed)))
    universe_total = int(universe.universe_total) if universe else total
    rotated_completed = max(0, bounded_completed - int(universe.seed_count)) if universe else bounded_completed
    rotating_total = max(0, universe_total - int(universe.seed_count)) if universe else universe_total
    absolute_covered = int(universe.cursor_start) + rotated_completed if universe else bounded_completed
    sweep_completed = absolute_covered % rotating_total if rotating_total else bounded_completed
    sweep_number = absolute_covered // rotating_total + 1 if rotating_total else 1
    if rotated_completed and rotating_total and sweep_completed == 0:
        sweep_completed = rotating_total
        sweep_number = max(1, absolute_covered // rotating_total)
    rotating_batch = max(1, int(universe.selected_dynamic_count)) if universe else max(1, total)
    batch_count = (rotating_total + rotating_batch - 1) // rotating_batch if rotating_total else 1
    batch_number = ((int(universe.cursor_start) % rotating_total) // rotating_batch) + 1 if universe and rotating_total else 1
    payload = {
        "run_id": run_id,
        "phase": str(phase)[:40],
        "symbols": [str(symbol).upper()[:12] for symbol in symbols[:240]],
        "current_ticker": str(current_ticker).upper()[:12],
        "completed": bounded_completed,
        "total": total,
        "percent": round((bounded_completed / total) * 100, 2) if total else 0,
        "message": str(message)[:240],
        "universe_total": universe_total,
        "sweep_completed": min(universe_total, sweep_completed + (int(universe.seed_count) if universe else 0)),
...
def analysis_symbols(symbols: list[str]) -> list[str]:
    ...
    for ticker in [*CONTEXT_BENCHMARKS, *SECTOR_ETFS]:
...
def evaluate(
    ...
    write_argentum_research_progress(
        "market_data",
        symbols,
        message=(
            f"Loading exchange batch {((dynamic.cursor_start % dynamic.universe_total) // max(1, dynamic.selected_dynamic_count)) + 1} "
            f"of {(dynamic.universe_total + max(1, dynamic.selected_dynamic_count) - 1) // max(1, dynamic.selected_dynamic_count)} "
            f"({len(symbols)} stocks from {dynamic.universe_total})."
            if dynamic
            else f"Loading market history for {len(symbols)} stocks."
        ),
        universe=dynamic,
    )
    try:
        ...
        write_argentum_research_progress("failed", symbols, message=f"Market history unavailable: {exc}", universe=dynamic)
    ...
    evaluation_at = datetime.now().astimezone()
    quant_snapshots = build_quant_snapshots(data, data.tickers, generated_at=evaluation_at)
    latest_ticker = ""

    def report_ticker_progress(ticker: str, completed: int, total: int) -> None:
        nonlocal latest_ticker
        latest_ticker = ticker or latest_ticker
        write_argentum_research_progress(
            "evaluate",
            symbols,
            current_ticker=latest_ticker,
            completed=completed,
            message=(
                f"Evaluating {latest_ticker} · {min(completed + 1, total)} of {total}."
                if completed < total
                else f"Finished evaluating {total} stocks."
            ),
            universe=dynamic,
        )
        if completed < total:
            try:
                hold_ms = max(0, min(2000, int(os.environ.get("STOCK_GURU_PROGRESS_HOLD_MS", "0"))))
            except ValueError:
                hold_ms = 0
            if hold_ms:
                time.sleep(hold_ms / 1000)

    evaluations = evaluate_market_data(
        data,
        settings,
    ...
    write_argentum_research_progress(
        ...
        message=f"Finished evaluating {len(symbols)} stocks.",
        universe=dynamic,
    )
    ...
    quant_path = write_quant_report(quant_snapshots)
    console.print(f"Wrote quantitative features: {quant_path}")
    macro_rates = fetch_macro_rate_context(now=evaluation_at)
    regime = build_market_regime(
        data,
        data.tickers,
        generated_at=evaluation_at,
        rates=market_regime_rate_values(macro_rates),
    )
    write_market_context_report(regime)
    persisted_research = load_research_json()
    institutional_changes: list[dict[str, object]] = []
    try:
        signal_payload = json.loads(COPY_SIGNALS_PATH.read_text()) if COPY_SIGNALS_PATH.exists() else {}
        raw_signals = signal_payload.get("signals", []) if isinstance(signal_payload, dict) else signal_payload
        institutional_changes = [item for item in raw_signals if isinstance(item, dict)] if isinstance(raw_signals, list) else []
    except Exception:
        institutional_changes = []
    analyses = {}
    for symbol in symbols:
        snapshot = quant_snapshots.get(symbol)
        if snapshot is None:
            continue
        research_item = persisted_research.get(symbol)
        events = build_catalyst_events(symbol, research_item.news_items, received_at=evaluation_at) if research_item else []
        ...
        score = score_symbol(snapshot, context, regime, generated_at=evaluation_at)
        analyses[symbol] = build_analysis_object(snapshot, context, score, regime)
    ranked = rank_analysis_objects(analyses, generated_at=evaluation_at)
    latest_analysis, daily_analysis = write_analysis_report(ranked)
    console.print(f"Wrote ranked analysis: {latest_analysis} (daily snapshot {daily_analysis})")
    if dynamic:
        commit_dynamic_universe(dynamic)
...
@app.command("quant-backtest")
def quant_backtest(
    tickers: Optional[List[str]] = typer.Option(
        None,
        "--tickers",
        "-t",
        help="Repeatable ticker option or comma-separated groups. Defaults to the required real-symbol validation set.",
    ),
    period: str = typer.Option("5y", "--period", help="Daily-history window used for the as-of validation."),
    step_sessions: int = typer.Option(20, "--step-sessions", min=1, max=120, help="Evaluate every Nth trading session."),
    minimum_history: int = typer.Option(200, "--minimum-history", min=50, max=500, help="Minimum prior bars before a score is eligible."),
    minimum_sample: int = typer.Option(30, "--minimum-sample", min=5, max=1000, help="Minimum observations before a bucket is trusted."),
    dry_run: bool = typer.Option(False, "--dry-run", help="Calculate and print metrics without writing the backtest artifact."),
) -> None:
    """Run leakage-safe quant validation with costs and an unseen walk-forward period."""
    symbols = resolve_tickers(tickers) if tickers else ["AAPL", "MSFT", "NVDA", "TSLA", "AMZN", "META", "SPY"]
    console.print(f"Running as-of quant validation for {len(symbols)} symbols...")
    data = download_history(analysis_symbols(symbols), period=period, interval="1d", spot_verify=True)
    ...
    if not report.observations:
        console.print("No eligible historical observations. The validator did not fabricate a result.")
        raise typer.Exit(2)
    walk_forward = walk_forward_validate(report, minimum_sample=minimum_sample)
    table = Table(title="Quant Backtest · Net 20-day outcomes after round-trip costs")
    for column in ["Score", "n", "Trusted", "Win", "Average", "Median", "vs SPY"]:
        table.add_column(column, justify="right" if column not in {"Score", "Trusted"} else "left")
    for bucket, horizons in report.score_buckets.items():
        metrics = horizons["20d"]
        table.add_row(
            bucket,
            str(metrics.sample_size),
            "yes" if metrics.trusted_sample else "no",
            research_pct(metrics.win_rate),
            research_pct(metrics.average_return),
            research_pct(metrics.median_return),
            research_pct(metrics.average_relative_return),
        )
    console.print(table)
    console.print(
        "Walk-forward: "
        f"threshold {walk_forward.selected_threshold:.0f} · validation n={walk_forward.validation_metrics.sample_size} · "
        f"net 20d {research_pct(walk_forward.validation_metrics.average_net_return_20d)} · "
        f"vs SPY {research_pct(walk_forward.validation_metrics.average_relative_return_20d)}"
    )
    for warning in walk_forward.warnings:
        console.print(f"Warning: {warning}")
    if not dry_run:
        path = write_quant_backtest_report(report, walk_forward=walk_forward)
        console.print(f"Wrote quant validation: {path}")
...
@app.command("intraday-context")
def intraday_context(
    tickers: Optional[List[str]] = typer.Option(None, "--tickers", "-t", help="Repeatable ticker option or comma-separated ticker groups."),
    dry_run: bool = typer.Option(False, "--dry-run", help="Print only; do not write the normalized context report."),
) -> None:
    """Build normalized 1m/5m/15m/1h/session/daily market context for a bounded shortlist."""
    ...
    try:
        ...
        console.print(f"Intraday context skipped: {exc}")
        return
    contexts = build_multi_timeframe_contexts(intraday_data, symbols, now=now, daily_data=daily_data)
    table = Table(title="Multi-Timeframe Market Context")
    for column in ["Symbol", "Feed", "Health", "Last", "VWAP", "RVOL", "Spread", "Alignment", "Conflicts"]:
        table.add_column(column)
    for symbol, item in contexts.items():
        table.add_row(
            symbol,
            item.source_provider,
            item.data_health_state,
            f"${item.last_price:.4f}" if item.last_price is not None else "—",
            f"${item.session_vwap:.4f}" if item.session_vwap is not None else "—",
            f"{item.relative_volume:.2f}" if item.relative_volume is not None else "—",
            f"{item.spread_pct:.3%}" if item.spread_pct is not None else "—",
            item.alignment,
            str(len(item.conflicts)),
        )
    console.print(table)
    if dry_run:
        return
    path = write_intraday_context_report(contexts)
    console.print(f"Wrote intraday context: {path}")
    macro_rates = fetch_macro_rate_context(now=now)
    regime = build_market_regime(daily_data, daily_symbols, generated_at=now, rates=market_regime_rate_values(macro_rates))
    regime_path = write_market_context_report(regime)
    console.print(f"Wrote market context: {regime_path}")
    console.print(f"Macro rates: {macro_rates.status} ({macro_rates.provider})")
...
def doctor(
    ...
    providers = [
        ...
        for name, enabled in [
            ...
            ("FRED", bool(provider_keys.fred_api_key)),
        ]
        if enabled
    ]
