...
from .multi_timeframe import build_multi_timeframe_contexts
...
def run_intraday_control_cycle(
    *,
    ...
    timeframe_contexts = build_multi_timeframe_contexts(
        data,
        list(dict.fromkeys([*symbols, *positions.keys()])),
        now=now,
        quotes=quotes,
    )

    def symbol_context(symbol: str) -> IntradayMarketContext:
        timeframe = timeframe_contexts.get(symbol)
        if timeframe is None:
            return replace(context, data_health_usable=False, timeframe_conflicts=("Multi-timeframe market context is unavailable.",))
        return replace(
            context,
            multi_timeframe_aligned=timeframe.alignment == "BULLISH",
            data_health_usable=timeframe.usable,
            timeframe_conflicts=timeframe.conflicts,
        )
    ...
    for symbol, position in positions.items():
        ...
        if indicators is None:
            ...
            continue
        decision = evaluate_intraday_exit(position, indicators, settings, quote=quotes.get(symbol), context=symbol_context(symbol), daily_risk=daily_risk)
