...
def test_stock_office_13f_watchlist_enables_the_verified_named_managers() -> None:
    ...
    assert {entry.cik for entry in entries} == {
        "0001067983", "0001350694", "0001336528", "0001649339",
        "0001536411", "0001029160", "0001656456", "0001697748",
    }
...
def test_change_signals_are_explicitly_delayed_research_references() -> None:
    ...
    assert all(item["issuer_name"] and item["security_identifier"] for item in signals)
    assert all("share_delta" in item and "as_filed_value" in item for item in signals)
...
def test_refresh_is_idempotent_preserves_other_importers_and_never_places_orders(tmp_path: Path) -> None:
    ...
    assert len(status["research_signals"]) == 3
    assert sum(item["ticker_resolved"] is False for item in status["research_signals"]) == 2
    assert all(item["source_id"] == "sec_13f" for item in status["research_signals"])
