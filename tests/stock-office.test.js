  ...
  writeJson(path.join(stockRoot, "data/provider_health.json"), {
    ...
    providers: {
      YAHOO_CHART: {
        ...
        status: "HEALTHY",
        last_status: "success",
        last_checked_at: "2026-06-19T11:45:00Z",
        last_success_at: "2026-06-19T11:45:00Z",
        last_latency_ms: 42,
        success_rate: 1,
        data_type: "OHLCV_HISTORY",
        interval: "1m",
        requested_symbols: ["BAC"],
        returned_symbols: ["BAC"],
      },
    },
  });
...
test("Stock Office loads local records without exposing secrets", () => {
  ...
  assert.equal(snapshot.providerHealth.providers[0].provider, "YAHOO_CHART");
  assert.equal(snapshot.providerHealth.providers[0].latencyMs, 42);
